...
nysyke <- Inklusjon[which(abs(difftime(Inklusjon$Diagnose_Klinisk_Dato, Inklusjon$InklusjonDato, units = 'days')) <= 30), ]
# nysyke <- Inklusjon[which(abs(difftime(Inklusjon$Diagnose_Klinisk_Dato, Inklusjon$InklusjonDato, units = 'days')) <= 30 & Inklusjon$Inklusjonsaar <= 2019), ]
...
tmp2 <- aux2 %>% group_by(Sykehusnavn.x, SkjemaGUID) %>% summarise(anca = ((1 %in% PR3AncaPositiv) | (0 %in% PR3AncaPositiv)))
...
print(xtable::xtable(proms, align= c('l','l',rep('r', ncol(proms)-1)), digits=1, caption='Gjennomsnitt av pasientsvar basert på alle registreringer, dvs.  at pre og postgruppen kan inneholde forskjellige individer.'), include.rownames = F)
...
print(xtable::xtable(proms, align= c('l','l',rep('r', ncol(proms)-1)), digits=1, caption='Gjennomsnitt av pasientsvar, med samme pasienter i Inklusjon -og oppfølgingsgruppen.'), include.rownames = F)
