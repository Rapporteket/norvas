...
\title{Figurer og tabeller til årsrapport for Norvas 2020}
\author{Norvas}
...
# Inklusjon_pguid <- read.table('I:/norvas/DataDump_MRS-PROD_Inklusjonskjema_2021-05-18_1058.csv', header=TRUE, sep=";",
#                               stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
...
outfile <- paste0('sykdomsvurdering_rap_aar_gr1.', utformat)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{sykdomsvurdering_rap_aar_gr1.pdf}
\caption{Sykdomsvurdering \Sexpr{rap_aar}, gruppe 1}
\end{figure}
...
# x11()
registreringer <- Inklusjon %>% group_by(Sykehusnavn) %>% summarise('Foer2019' = sum(Inklusjonsaar<rap_aar),
                                                  'aar2019' = sum(Inklusjonsaar==rap_aar))
...
legend('bottomright', legend = c(paste0('T.o.m. ', rap_aar-1, ', N=', sum(registreringer$Foer2019, na.rm = T)),
                                 paste0(rap_aar, ', N=', sum(registreringer$aar2019, na.rm = T))), col = figinfo$farger[c(3,1)],
       pch = 15, border = NA, bty='n')
title(main = 'Antall inklusjoner pr. HF')
if ( outfile != '') {dev.off()}

oppfolginger <- Oppfolging %>% group_by(Sykehusnavn) %>% summarise('Foer2019' = sum(Oppfolgingsaar<rap_aar),
                                                  'aar2019' = sum(Oppfolgingsaar==rap_aar))
...
pos <- barplot(plotMatrise[c(2,1), ], beside = F, horiz = T, col = figinfo$farger[c(3,1)], border=NA, xlab="Antall pasienter")
mtext(at=pos+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
legend('bottomright', legend = c(paste0('T.o.m. ', rap_aar-1, ', N=', sum(oppfolginger$Foer2019, na.rm = T)),
                                 paste0(rap_aar, ', N=', sum(oppfolginger$aar2019, na.rm = T))),
...
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
outfile <- paste0('reg_pr_diaggr.', utformat)
# x11()
figinfo <- rapFigurer::figtype(outfile = outfile)
grtxt <- registreringer$Sykehusnavn
vmarg <- min(1,max(0, strwidth(grtxt, units='figure', cex=cexgr)*0.75))
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{reg_pr_diaggr.pdf}
...
# ######## gr. 1 ################
indMedisinert2019 <- which(((Medisiner$Med_StartDato <= datoTil & is.na(Medisiner$Med_SluttDato)) | (Medisiner$Med_StartDato <= datoTil & Medisiner$Med_SluttDato >= datoFra)) & Medisiner$Diag_gr_nr == 1)

pid_Medisinert2019 <- unique(Medisiner$PasientGUID[indMedisinert2019])
pid_oppfolging2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar==rap_aar & Oppfolging$Diag_gr_nr==1])
pid_inkludert2019 <- unique(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar<=rap_aar & Inklusjon$Diag_gr_nr == 1])

pid_alle_pas2019 <- unique(c(pid_Medisinert2019, pid_oppfolging2019, pid_inkludert2019))

ant_pas_pr_med <- Medisiner[indMedisinert2019, ] %>% group_by(Sykehusnavn, Medikamentgruppe) %>%
...
figinfo <- rapFigurer::figtype(outfile = outfile)
par('fig'=c(vmarg, 1, 0, 1))
pos <- barplot(ant_pas_pr_med_gr1$`Biologiske legemidler`/ant_pas_pr_med_gr1$N*100, horiz = T, col = figinfo$farger[1], border=NA, xlab="Andel pasienter (%)")
mtext(at=pos+0.00, text=paste0(grtxt, ' (N=', ant_pas_pr_med_gr1$N, ')'), side=2, las=1, cex=cexgr, adj=1, line=0.25)
title(main = c('Biologiske legemidler,', 'storkarsvaskulitter'))
if ( outfile != '') {dev.off()}

# ######## gr. 2 ANCA ################
indMedisinert2019 <- which(((Medisiner$Med_StartDato <= datoTil & is.na(Medisiner$Med_SluttDato)) | (Medisiner$Med_StartDato <= datoTil & Medisiner$Med_SluttDato >= datoFra)) & Medisiner$Diag_gr_nr == 2)

pid_Medisinert2019 <- unique(Medisiner$PasientGUID[indMedisinert2019])
pid_oppfolging2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar==rap_aar & Oppfolging$Diag_gr_nr == 2])
pid_inkludert2019 <- unique(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar<=rap_aar & Inklusjon$Diag_gr_nr == 2])
...
figinfo <- rapFigurer::figtype(outfile = outfile)
par('fig'=c(vmarg, 1, 0, 1))
pos <- barplot(ant_pas_pr_med_gr2$`Biologiske legemidler`/ant_pas_pr_med_gr2$N*100, horiz = T, col = figinfo$farger[1], border=NA, xlab="Andel pasienter (%)")
mtext(at=pos+0.00, text=paste0(grtxt, ' (N=', ant_pas_pr_med_gr2$N, ')'), side=2, las=1, cex=cexgr, adj=1, line=0.25)
title(main = c('Biologiske legemidler,', 'ANCA assosiert vaskulitt'))
if ( outfile != '') {dev.off()}
## ######## gr. 3 Andre vaskulitter ################

indMedisinert2019 <- which(((Medisiner$Med_StartDato <= datoTil & is.na(Medisiner$Med_SluttDato)) | (Medisiner$Med_StartDato <= datoTil & Medisiner$Med_SluttDato >= datoFra)) & Medisiner$Diag_gr_nr == 3)

pid_Medisinert2019 <- unique(Medisiner$PasientGUID[indMedisinert2019])
pid_oppfolging2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar==rap_aar & Oppfolging$Diag_gr_nr == 3])
pid_inkludert2019 <- unique(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar<=rap_aar & Inklusjon$Diag_gr_nr == 3])
...
figinfo <- rapFigurer::figtype(outfile = outfile)
par('fig'=c(vmarg, 1, 0, 1))
pos <- barplot(ant_pas_pr_med_gr3$`Biologiske legemidler`/ant_pas_pr_med_gr3$N*100, horiz = T, col = figinfo$farger[1], border=NA, xlab="Andel pasienter (%)")
mtext(at=pos+0.00, text=paste0(grtxt, ' (N=', ant_pas_pr_med_gr3$N, ')'), side=2, las=1, cex=cexgr, adj=1, line=0.25)
title(main = c('Biologiske legemidler,', 'andre vaskulitter'))
if ( outfile != '') {dev.off()}
...
ant_pas_pr_med_foer2019 <- Medisiner[Medisiner$Med_StartDato < datoFra, ] %>% group_by(LegemiddelGenerisk) %>% summarise(Antall = length(unique(PasientGUID))) %>% ungroup()
# N_foer2019 <- length(unique(Medisiner[Medisiner$Med_StartDato < '2019-01-01', "PasientGUID"]))
indMedisinert_foer2019 <- which(Medisiner$Med_StartDato < datoFra)
pid_Medisinert_foer2019 <- unique(Medisiner$PasientGUID[indMedisinert_foer2019])
pid_oppfolging_foer2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar<rap_aar])
pid_inkludert_foer2019 <- unique(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar<rap_aar])
N_foer2019 <- length(unique(c(pid_Medisinert_foer2019, pid_oppfolging_foer2019, pid_inkludert_foer2019)))

ant_pas_pr_med_2019 <- Medisiner[(Medisiner$Med_SluttDato >= datoFra | is.na(Medisiner$Med_SluttDato)) & Medisiner$Med_StartDato < datoFra, ] %>% group_by(LegemiddelGenerisk) %>% summarise(Antall = length(unique(PasientGUID))) %>% ungroup()

indMedisinert2019 <- which((Medisiner$Med_SluttDato >= datoFra | is.na(Medisiner$Med_SluttDato)) & Medisiner$Med_StartDato < datoFra)
pid_Medisinert2019 <- unique(Medisiner$PasientGUID[indMedisinert2019])
pid_oppfolging2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar==rap_aar])
pid_inkludert2019 <- unique(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar<=rap_aar])
...
legend('bottomright', legend = c(paste0('T.o.m. ', rap_aar-1, ', N=', N_foer2019), paste0(rap_aar, ', N=', N_2019)),
       col = figinfo$farger[c(3,1)], pch = 15, border = NA, bty='n')
...
ant_pas_pr_med_foer2019 <- Medisiner[Medisiner$Med_StartDato < datoFra, ] %>% group_by(Medikamentgruppe) %>% summarise(Antall = length(unique(PasientGUID))) %>% ungroup()
# N_foer2019 <- length(unique(Medisiner[Medisiner$Med_StartDato < '2019-01-01', "PasientGUID"]))
indMedisinert_foer2019 <- which(Medisiner$Med_StartDato < datoFra)
pid_Medisinert_foer2019 <- unique(Medisiner$PasientGUID[indMedisinert_foer2019])
pid_oppfolging_foer2019 <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar<rap_aar])
...
ant_pas_pr_med_2019 <- Medisiner[(Medisiner$Med_SluttDato >= datoFra | is.na(Medisiner$Med_SluttDato)) & Medisiner$Med_StartDato < datoFra, ] %>% group_by(Medikamentgruppe) %>% summarise(Antall = length(unique(PasientGUID))) %>% ungroup()
...
ant_pas_pr_med <- merge(ant_pas_pr_med_foer2019, ant_pas_pr_med_2019, by = 'Medikamentgruppe', all = T)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{pasPaaMedisin.pdf}
...
print(xtable::xtable(Gj.sn_ant_oppf_pr_pasient_pr_hf, align= c('l','r','r','r'), caption=paste0('Gjennomsnittlig antall oppfølginger per pasient. Tallene i parantes angir antall pasienter aktuell for oppfølging, dvs. inkludert før ', rap_aar)), include.rownames = T)
...
outfile <- paste0("andel_2_oppf.", utformat)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_2_oppf.pdf}
...
ant_nysyke2019 <-  as.data.frame(table(nysyke$Diag_gr[nysyke$Inklusjonsaar==rap_aar]))
names(ant_nysyke2019) <- c('Diagnosegruppe', 'Antall')
print(xtable::xtable(ant_nysyke2019, align= c('l','l','r'), caption=paste0('Antall nysyke ', rap_aar)), include.rownames = F)
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("bvas_ved_oppf_anca_og_andre.", utformat)
norvasIndikator(figurdata[which(figurdata$Oppfolgingsaar==rap_aar),],
                tittel = c("Andel oppfølginger med utfylt BVAS", "for ANCA-assosierte vaskulitter",
                                      "og gruppe Andre vaskulitter"), outfile = outfile,
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("kerr_ved_oppf_gvv.", utformat)
norvasIndikator(figurdata[which(figurdata$Oppfolgingsaar==rap_aar),],
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{bvas_ved_oppf_anca_og_andre.pdf}
\caption{}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{kerr_ved_oppf_gvv.pdf}
\caption{}
\end{figure}
...
outfile <- paste0("anca_v_debut_ancaass_vas.", utformat)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{anca_v_debut_ancaass_vas.pdf}
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("remisjon_6mnd_anca.", utformat)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{remisjon_6mnd_anca.pdf}
...
outfile <- paste0("remisjon_6mnd_storkar_v2.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{remisjon_6mnd_storkar_v2.pdf}
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
outfile <- paste0("lav_prednisolon_gvv.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
outfile <- paste0("lav_prednisolon_gvv_kerr.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
                tittel = c("Andel pasienter med storkarsvaskulitt",
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{lav_prednisolon_anca.pdf}
\caption{Andel pasienter med ANCA assosiert vaskulitt på Prednisolon $\leq$ 5mg 6 mndr etter debut}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{lav_prednisolon_gvv.pdf}
\caption{Andel pasienter med storkarsvaskulitt på Prednisolon $\leq$ 7.5mg 6 mndr etter debut}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{lav_prednisolon_gvv_kerr.pdf}
\caption{Andel pasienter med storkarsvaskulitt på Prednisolon $\leq$ 7.5mg 6 mndr etter debut}
\end{figure}
...
Alvorlig_infeksjon2019 <- Alvorlig_infeksjon[Alvorlig_infeksjon$SelvrapportertAlvorligInfeksjon_Registrert_Dato >= datoFra &
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("ct_thorax.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
figurdata <- jalla
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
outfile <- paste0("ctmr_bihuler.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("ulctmr_storkar.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
# x11()
outfile <- paste0("utredet_GCA.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{ct_thorax.pdf}
\caption{Andel utført CT thorax for ANCA assosierte vaskulitter, alle tider t.o.m. 2020}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{ctmr_bihuler.pdf}
\caption{Andel utført CT/MR bihule for ANCA assosierte vaskulitter, alle tider t.o.m. 2020}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{ulctmr_storkar.pdf}
\caption{Andel med utført UL/CT/MR av mellomstore/store kar ved debut av storkarsvaskulitt (Large vessel vasculitis=LVV) , alle tider t.o.m. 2020}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{utredet_GCA.pdf}
\caption{Andel med utført Biopsi/UL/MR ved debut av Kjempecelle arteritt (Kraniell Giant cell arteritis= GCA), alle tider t.o.m. 2020}
\end{figure}


<<'PROM sykehusvisning', results='asis', echo=FALSE, warning=F>>=
nysyke <- Inklusjon[which(abs(difftime(Inklusjon$Diagnose_Klinisk_Dato, Inklusjon$InklusjonDato, units = 'days')) <= 30 |
...
cexgr <- 1
outfile <- paste0("tretthet.", utformat)
figinfo <- rapFigurer::figtype(outfile = outfile)
...
cexgr <- 1
outfile <- paste0("globalsykdomsaktivitet.", utformat)
figinfo <- rapFigurer::figtype(outfile = outfile)
farger <- figinfo$farger
...
cexgr <- 1
outfile <- paste0("smerter.", utformat)
figinfo <- rapFigurer::figtype(outfile = outfile)
farger <- figinfo$farger
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{tretthet.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{globalsykdomsaktivitet.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{smerter.pdf}
...
write.csv2(Indikatorer, 'I:/norvas/norvas_ind_20210607.csv', na = '', fileEncoding = 'UTF-8', row.names = F)
