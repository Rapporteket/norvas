...
print(xtable::xtable(
  ...
  caption=paste0('Gjennomsnittlig antall oppfølginger per pasient. Tallene i
                 parantes angir antall pasienter aktuell for oppfølging, dvs.
                 inkludert før ', rap_aar, ' og ikke ekskludert innen ', rap_aar)), include.rownames = F)
...
figurdata$Sykehusnavn <-
  kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
@
...
prednisolon_6mnd_v1 <- Inklusjon %>%
  select(Diagnose_ny_30,InklusjonDato, SkjemaGUID,
         Diag_gr_nr, Sykehusnavn, UnitId, PasientGUID, Diagnose_Klinisk_Dato) %>%
  filter(Diag_gr_nr==2) %>%
  mutate(
...
v6mnd <- prednisolon_6mnd_v1 |>
  filter(Med_StartDato %within% Ved_6mnd_int) |>
  dplyr::filter(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato %m+% months(6))) ==
                  min(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato %m+% months(6)))),
                .by = PasientGUID)
...
Tabell_v1 <- prednisolon_6mnd_v1 %>%
  ...
  # filter(Aar > rap_aar-3) |>
  tidyr::pivot_wider(names_from = Aar, values_from = c(Antall, N), names_sort = TRUE)
write.csv2(Tabell_v1, paste0(figfolder, "lav_prednisolon_anca_v1.csv"),
...
Indikatorer <- prednisolon_6mnd_v1 %>%
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) %>%
  select(UnitId, year, var, denominator, ind_id) %>%
...
outfile <- paste0(figfolder, "lav_prednisolon_anca_v1.", utformat)
...
norvasIndikator(prednisolon_6mnd_v2 |> filter(Aar<=rap_aar),
                tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
                           "på Prednisolon <= 5mg 6 måneder etter debut"),
                maal = 60, minstekrav = 30, outfile = outfile, xmax = 100)
# outfile <- paste0(figfolder, "lav_prednisolon_anca_aar_v1.", utformat)
# norvasFigIndikatorAar(prednisolon_6mnd_v1 |> filter(Aar<=rap_aar),
#                 tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
#                            "på Prednisolon <= 5mg 6 måneder etter debut"),
#                 maal = 60, minstekrav = 30, outfile = outfile, xmax = 100)
# outfile <- paste0(figfolder, "lav_prednisolon_anca_aar_v2.", utformat)
# norvasFigIndikatorAar(prednisolon_6mnd_v2 |> filter(Aar<=rap_aar),
#                 tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
#                            "på Prednisolon <= 5mg 6 måneder etter debut"),
#                 maal = 60, minstekrav = 30, outfile = outfile, xmax = 100)
...
# prednisolon_6mnd <- Inklusjon %>%
#   select(Diagnose_ny_30,InklusjonDato, Inklusjonsaar, SkjemaGUID,
#          Diag_gr_nr, Sykehusnavn, UnitId, PasientGUID, Diagnose_Klinisk_Dato) %>%
#   filter(Diagnose_ny_30 == 1 &
#            Diag_gr_nr==1 &
#            InklusjonDato <= datoTil) %>%
#   mutate(Ved_6mnd_int = lubridate::interval(
#     Diagnose_Klinisk_Dato %m+% months(6),
#     Diagnose_Klinisk_Dato %m+% months(8))) %>%
#   merge(Medisiner %>% filter(LegemiddelGenerisk == "Prednisolon") %>%
#           select(HovedskjemaGUID, Med_StartDato,
#                  Med_SluttDato, Dose),
#         by.x = "SkjemaGUID", by.y = "HovedskjemaGUID") %>%
#   filter(Med_StartDato %within% Ved_6mnd_int) %>%
#   filter(Dose == min(Dose), .by = PasientGUID) %>%
#   filter(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato %m+% months(6))) ==
#            min(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato %m+% months(6)))),
#          .by = PasientGUID)
#
# Tabell <- prednisolon_6mnd %>%
#   dplyr::summarise(ant_pred_under_5 = sum(Dose <= 7.5),
#                    N = dplyr::n(),
#                    .by = Sykehusnavn) %>%
#   janitor::adorn_totals() %>%
#   dplyr::mutate(Andel = ant_pred_under_5/N*100) %>%
#   dplyr::mutate(Andel = ifelse(N < 5, NA, Andel)) %>%
#   dplyr::arrange(-Andel)
#
#
# ################ Data til Resultatportalen ###################################
# ind7_Andelstorvas_Prednisolon_NorVas_kerr <- prednisolon_6mnd %>%
#   dplyr::select(Sykehusnavn, UnitId, Inklusjonsaar, Dose) %>%
#   dplyr::rename(Aar = Inklusjonsaar) %>%
#   dplyr::mutate(Teller = as.numeric(Dose <= 7.5)) %>%
#   dplyr::mutate(Nevner = 1)
# ind7_Andelstorvas_Prednisolon_NorVas_kerr$ind_id <- "norvas_storvas_prednisolon"
# Indikatorer <- ind7_Andelstorvas_Prednisolon_NorVas_kerr %>%
#   rename(year = Aar,
#          var = Teller,
#          denominator = Nevner) %>%
#   select(UnitId, year, var, denominator, ind_id) %>%
...
#
#
# figurdata <- ind7_Andelstorvas_Prednisolon_NorVas_kerr
...
etter debut. Gjelder alle år t.o.m. \Sexpr{rap_aar}. \\ Utvalg: Pasienter som har en registrering av prednisolon $\pm$ 4 uker av 6 måneder etter diagnosedato. }
\end{figure}
...
nysyke <- Pasientsvar[, c("PasientGUID", "FormDate", "SkjemaGUID",
                          "Prom_Tretthet",
...
tidfrainklusjon<- Inklusjon %>% dplyr::filter(Diagnose_ny_30 == 1) %>%
  select(SkjemaGUID, InklusjonDato, FormStatus) %>%
  merge(Oppfolging[, c("HovedskjemaGUID", "OppfolgingsDato", "SkjemaGUID", "UnitId",
                       "FormStatus")],
        by.x = "SkjemaGUID", by.y = "HovedskjemaGUID",
        suffixes = c("", "_oppf")) %>%
  mutate(tid_inkl_oppf = difftime(OppfolgingsDato, InklusjonDato, units = "days") %>%
...
tidfrainklusjon_forste <- tidfrainklusjon %>%
  filter(OppfolgingsDato==first(OppfolgingsDato, order_by = OppfolgingsDato), .by = SkjemaGUID)
...
norvasFigAndeler(RegData=tidfrainklusjon, valgtVar=valgtVar, datoFra="2000-01-01", datoTil=datoTil,
                 minald=minald, maxald=maxald, erMann=erMann, outfile=outfile, reshID=reshID,
                 enhetsUtvalg=enhetsUtvalg, preprosess=preprosess,
                 valgtShus=valgtShus,hentData=hentData, datovar="InklusjonDato", aldervar=aldervar)

outfile <- paste0(figfolder, 'tid_inklu_oppf_forste.', utformat)
norvasFigAndeler(RegData=tidfrainklusjon_forste, valgtVar=valgtVar, datoFra="2000-01-01", datoTil=datoTil,
                 minald=minald, maxald=maxald, erMann=erMann, outfile=outfile, reshID=reshID,
                 enhetsUtvalg=enhetsUtvalg, preprosess=preprosess,
                 valgtShus=valgtShus,hentData=hentData, datovar="InklusjonDato", aldervar=aldervar)
