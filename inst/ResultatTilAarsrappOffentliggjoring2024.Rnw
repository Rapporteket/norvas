...
\title{Figurer og tabeller til årsrapport for Norvas 2024}
\author{Norvas}
...
write.csv2(Indikatorer,
           paste0("~/regdata/norvas/behandlingskvalitet/Indikatorer_", Sys.Date(), ".csv"),
           row.names = F)

# tmp <- dg_ny |> filter(year==2024,
#                        ind_id == "norvas_dg_alle")

# Indikatorer <- Indikatorer |>
#   filter(ind_id %in% c("norvas_storvas_prednisolon", "norvas_anca_prednisolon",
#                        "norvas_storkar_remisjon", "norvas_anca_remisjon",
#                        "norvas_anca_ved_debut", "norvas_kerr", "norvas_bvas")) |>
#   bind_rows(dg_samlet) |>
#   dplyr::filter(year <= rap_aar)
#
# # write.csv2(Indikatorer, "~/GIT/norvas/doc/Indikatorer_tom2022.csv", row.names = F)
#
# write.csv2(Indikatorer, paste0("~/GIT/norvas/doc/Indikatorer_", Sys.Date(), ".csv"), row.names = F)
@
