...
norvasFigFordelingGr(RegData=Inklusjon, valgtVar='ErMann',
                     ...
                     aldervar='PatientAge', diag_gruppe=99)
...
tabell_remisjon_6mnd_aar_raa <- ind4_AndelAnCA_remisjon_NorVas |>
  filter(Aar <= rap_aar) |>
  summarise(Antall = sum(Teller),
            N = n(),
            .by = c(Sykehusnavn, Aar)) |>
...
print(xtable::xtable(
  ...
  caption=paste0(
    ...
    KERR ved 6 mnd. plusminus 4 uker etter diagnosedato inngår.')), include.rownames = F)
...
#            Med_StartDato %within% Ved_start_int)
#
# start <- prednisolon_6mnd_v1 |>
#   filter(Med_StartDato %within% Ved_start_int) |>
#   dplyr::filter(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato)) ==
#                   min(abs(difftime(Med_StartDato, Diagnose_Klinisk_Dato))),
#                 .by = PasientGUID)
# v6mnd <- prednisolon_6mnd_v1 |>
#   filter(Med_StartDato %within% Ved_6mnd_int) |>
...
#                 .by = PasientGUID)
#
# prednisolon_6mnd_v1 <- merge(
#   start |> select(PasientGUID, UnitId, Sykehusnavn, Diagnose_Klinisk_Dato, ),
#   v6mnd |> select(PasientGUID, Dose),
#   by = "PasientGUID") |>
...
Tabell_v2 <- prednisolon_6mnd_v2 |>
  ...
  # filter(Aar > rap_aar-3) |>
  tidyr::pivot_wider(names_from = Aar, values_from = c(Antall, N), names_sort = TRUE)
write.csv2(Tabell_v2, paste0(figfolder, "lav_prednisolon_anca_v2.csv"),
...
Tabell_v2 <- prednisolon_6mnd_v2 |>
  ...
  group_modify(~ .x |> janitor::adorn_totals()) |>
  # filter(Aar > rap_aar-3) |>
  tidyr::pivot_wider(names_from = Aar, values_from = c(Antall, N),
                     names_sort = TRUE)
write.csv2(Tabell_v2, paste0(figfolder, "lav_prednisolon_gvv_v2.csv"),
           row.names = F, fileEncoding = "Latin1", na = "")
...
# outfile <- paste0(figfolder, "lav_prednisolon_gvv_v1.", utformat)
# norvasIndikator(prednisolon_6mnd_v1 |> filter(Aar<=rap_aar),
...
# Indikatorer <- indikator_residiv_kerr |>
#   rename(year = Aar,
#          var = Teller,
#          denominator = Nevner) |>
#   mutate(ind_id = "norvas_residiv_kerr") |>
#   select(UnitId, year, var, denominator, ind_id) |>
#   bind_rows(Indikatorer)
...
\caption{Gjennomsnittlig pasientrapportert tretthet ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}globalsykdomsaktivitet.pdf}
\caption{Gjennomsnittlig pasientrapportert global sykdomsaktivitet ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}smerter.pdf}
\caption{Gjennomsnittlig pasientrapportert smerte ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}.}
\end{figure}
