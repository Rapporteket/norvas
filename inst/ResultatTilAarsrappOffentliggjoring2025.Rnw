...
\title{Figurer og tabeller til årsrapport for Norvas 2025}
\author{Norvas}
...
# write.csv2(Inklusjon,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Inklusjon.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(Oppfolging,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Oppfolging.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(Diagnoser,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Diagnoser.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(Medisiner,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Medisiner.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(BVAS,
#            "C:/Users/kth200/regdata/norvas/utleveringer/BVAS.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(KERR,
#            "C:/Users/kth200/regdata/norvas/utleveringer/KERR.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(VDI,
#            "C:/Users/kth200/regdata/norvas/utleveringer/VDI.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(
#   Alvorlig_infeksjon,
#   "C:/Users/kth200/regdata/norvas/utleveringer/Alvorlig_infeksjon.csv",
#   row.names = F, fileEncoding = "Latin1")
# write.csv2(Utredning,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Utredning.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(Labskjema,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Labskjema.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(Pasientsvar,
#            "C:/Users/kth200/regdata/norvas/utleveringer/Pasientsvar.csv",
#            row.names = F, fileEncoding = "Latin1")
# write.csv2(
#   VaskulittIntervensjon,
#   "C:/Users/kth200/regdata/norvas/utleveringer/VaskulittIntervensjon.csv",
#   row.names = F, fileEncoding = "Latin1")
...
#     "Drammen" , 974631326,
#     "Haukeland" ,  974557746,
#     "Lillehammer" , 985773238,
#     "UNN" , 983974899,
#     "Ålesund" , 974747138,
#     "Ullevål sykehus" , 974589095,
#     'Rikshospitalet' , 874716782,
#     "Kristiansand" , 974733013,
#     "Nordlandsykehuset" , 974795361,
#     "Førde" , 974744570,
#     "Betanien" , 981275721,
#     "St. Olavs" , 974749025,
#     "Haugesund" , 973156829,
#     "Levanger" , 974754118,
#     "Helgelandssykehuset" , 974795515,
#     "Martina Hansen" , 985962170,
#     'Moss' , 974633698,
#     'Stavanger' , 974703300,
#     'OUS HF' , 993467049
#   )
...
figfolder <- "C:/Users/kth200/regdata/norvas/aarsrapp2025/figfolder/"
if (!dir.exists(figfolder)) {
  dir.create(figfolder)
}
# knitr::knit2pdf("ResultatTilAarsrappOffentliggjoring2024.Rnw")
@


<<'Fig: bakgrunnsdata', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
...
diagnosefordeling <- Diagnoser |>
  merge(aux, by = "PasientGUID") |>
  filter(Diagnose_Klinisk_Dato < paste0(rap_aar+1, "-01-01")) |>
  summarise(ant_rapaar = sum(eksklusjonsaar >= rap_aar),
            ant_foer_rapaar = sum(eksklusjonsaar < rap_aar),
            N = n(),
            .by = "Diagnose") |>
  arrange(N)

plotMatrise <- t(as.matrix(diagnosefordeling[,c(2,3)]))
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
outfile <- paste0(figfolder, 'diagnosefordeling_splittet.', utformat)
grtxt <- diagnosefordeling$Diagnose |> norvas::wrap.it(28)
legendTxt <- c(paste0('Følges opp i ', rap_aar, ', N=', sum(diagnosefordeling$ant_rapaar, na.rm = T)),
               paste0('Ekskludert før ', rap_aar, ', N=', sum(diagnosefordeling$ant_foer_rapaar, na.rm = T)))
norvarStabelGrvar(plotMatrise=plotMatrise/sum(diagnosefordeling$N)*100,
                  grtxt = grtxt,
                  outfile = outfile,
                  tittel = "Diagnosefordeling",
                  xlab =" Andel pasienter (%)",
                  legendTxt = legendTxt,
                  pstTxt = paste0(round(diagnosefordeling$N/sum(diagnosefordeling$N)*100, 1), "%"),
                  NTxt = diagnosefordeling$N)

@

<<'Tab: Diagnosefordeling', results='asis', echo=FALSE, warning=F>>=
diagnosefordeling <- diagnosefordeling |> arrange(desc(N))
names(diagnosefordeling)[2:4] <- c(paste0("Følges opp ", rap_aar),
                                   paste0("Ekskludert før ", rap_aar),
                                   "Totalt")

print(xtable::xtable(
  diagnosefordeling, align= c('l','L{2.5in}','r','r', 'r'),
  caption=paste0('Antall av de inkluderte som har de ulike diagnosene. Totalt og
                 splittet på de som følges opp i ', rap_aar, ' og de som er ekskluderte.
                 Definisjonen på at en pasient følges opp i ', rap_aar, ' er at det ikke er
                 registrert en eksklusjonsdato før ', rap_aar)),
  include.rownames = F)


reg3aar <- Inklusjon |>
  dplyr::mutate(DiagnoseAar = lubridate::year(Diagnose_Klinisk_Dato)) |>
  dplyr::filter(DiagnoseAar %in% (rap_aar-2):rap_aar,
                Diagnose_ny_30 == 1) |>
...
print(xtable::xtable(
  reg3aar, align= c('l','l','r', 'r'),
  caption = "Oversikt over antall nysyke pasienter med AAV og LVV. Nysyke er definert ved at pasienten enten er registrert som  Inkludert med ny diagnose eller at det er 30 dager eller mindre mellom inklusjonsdato og diagnosedato (så fremt pasienten ikke er registrert som inkludert med etablert diagnose). "),
  include.rownames = F)


reg3aar_shus <- Inklusjon |>
  dplyr::mutate(DiagnoseAar = lubridate::year(Diagnose_Klinisk_Dato),
                Diagnose_ny = factor(Diagnose_ny_30, levels=0:1,
                                     labels=c("etablert", "nysyk"))) |>
  dplyr::filter(DiagnoseAar %in% (rap_aar-2):rap_aar,
                Diag_gr_nr == 1) |>
  dplyr::count(DiagnoseAar, Sykehusnavn, Diagnose_ny) |>
  dplyr::arrange(DiagnoseAar, Diagnose_ny) |>
  tidyr::pivot_wider(names_from = c(DiagnoseAar, Diagnose_ny),
                     names_sep = ".", values_from = n, values_fill = 0) |>
  janitor::adorn_totals()

print(xtable::xtable(
  reg3aar_shus, align= c('l','L{1.5in}',rep('r', dim(reg3aar_shus)[2]-1)),
  caption = "Oversikt over antall nysyke og pasienter med etablert diagnose med LVV. Nysyke er definert ved at pasienten enten er registrert som Inkludert med ny diagnose eller at det er 30 dager eller mindre mellom inklusjonsdato og diagnosedato (så fremt pasienten ikke er registrert som inkludert med etablert diagnose). År angir diagnoseår."),
  include.rownames = F)
...
print(xtable::xtable(
  reg3aar_shus, align= c('l','L{1.5in}',rep('r', dim(reg3aar_shus)[2]-1)),
  caption = "Oversikt over antall nysyke og pasienter med etablert diagnose med AAV. Nysyke er definert ved at pasienten enten er registrert som  Inkludert med ny diagnose eller at det er 30 dager eller mindre mellom inklusjonsdato og diagnosedato (så fremt pasienten ikke er registrert som inkludert med etablert diagnose). År angir diagnoseår."),
  include.rownames = F)


Alder_v_diagnose <- RegData |>
  dplyr::filter(lubridate::year(Diagnose_Klinisk_Dato) <= rap_aar ) |>
  dplyr::mutate(Alder = cut(DiagnoseAlder, breaks = c(seq(16,76,by=10), 140),
                            include.lowest = T, right = F)) |>
  dplyr::count(Diagnose, Alder) |>
  dplyr::group_by(Diagnose) |>
  dplyr::group_modify(~ .x |> janitor::adorn_totals()) |>
  tidyr::pivot_wider(names_from = Alder, values_from = n, values_fill = 0) |>
  dplyr::mutate(dplyr::across(1:7, ~ ./ Total * 100)) |>
  dplyr::mutate(dplyr::across(1:7, ~ paste0(sprintf("%.1f", .), " %"))) |>
  dplyr::rename(N = Total)

print(xtable::xtable(
  Alder_v_diagnose,
  align= c('l','L{1.7in}',rep('r', dim(Alder_v_diagnose)[2]-1)),
  caption = paste0("Aldersfordeling ved diagnosetidspunkt. T.o.m. ", rap_aar)),
  include.rownames = F)
...
<<'Fig: bakgrunnsdata 2', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
aldervar='PatientAge'
valgtVar='DiagnoseAlder'

outfile <- paste0(figfolder, 'diagnosealder_gr1.', utformat)
norvasFigAndeler(RegData=RegData, valgtVar=valgtVar,
...
# Inklusjon$diag_fact <- as.factor(RegData$Diagnose)
# outfile <- paste0(figfolder, 'kjonn.', utformat)
# norvasFigFordelingGr(RegData=Inklusjon, valgtVar='ErMann',
#                      datoFra='2014-01-01', datoTil=datoTil,
#                      minald=0, maxald=130, erMann=99, outfile=outfile,
#                      reshID=0, enhetsUtvalg=0, preprosess=F,
#                      valgtShus=c(''),hentData=F, datovar='InklusjonDato',
#                      aldervar='PatientAge', diag_gruppe=99, grvar = "diag_fact")

outfile <- paste0(figfolder, 'kjonn.', utformat)
Inklusjon$ErKvinne <- 1 - Inklusjon$ErMann
norvasFigAndelGruppert(RegData=Inklusjon,
                       var="ErKvinne",
                       grvar="Diagnose",
...
datovar='BVAS_Dato'
norvasFigAndeler(RegData=BVAS, valgtVar=valgtVar,
                 datoFra=datoFra, datoTil=paste0(rap_aar-1, "-12-31"),
                 minald=minald, maxald=maxald, erMann=erMann,
                 outfile=outfile, reshID=reshID,
                 enhetsUtvalg=enhetsUtvalg, preprosess=preprosess,
                 diag_gruppe=99,
                 valgtShus=valgtShus,hentData=hentData,
                 datovar=datovar, aldervar=aldervar, inkl_N = T)

datoFra <- paste0(rap_aar, "-01-01")
datoTil=paste0(rap_aar, "-12-31")

outfile <- paste0(figfolder, 'sykdomsvurdering_rapaar_gr2.', utformat)
norvasFigAndeler(RegData=BVAS[BVAS$BVAS_Dato >=
                                datoFra & BVAS$BVAS_Dato <= datoTil, ],
                 valgtVar=valgtVar,
                 datoFra=datoFra, datoTil=datoTil,
                 minald=minald, maxald=maxald, erMann=erMann,
                 outfile=outfile, reshID=reshID,
...
norvasFigAndeler(RegData=KERR, valgtVar="Sykdomsvurdering_kerr",
                 ...
                 diag_gruppe=1,
                 valgtShus=valgtShus,hentData=hentData,
                 datovar="KerrsKriterier_Dato", aldervar=aldervar, inkl_N = T)

## Ønske om å se over flere år

## Se på mulighet for å sammenligne sykehus Stabel i Wenches årsrapport.
## Kun for HF med mer enn 50 registreringer i _rapaar.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}diagnosefordeling_splittet.pdf}
\caption{Diagnoser i Norvas blant de som følges opp og blant de ekskluderte}
\end{figure}
...
\caption{Kjønnsfordeling etter diagnose}
\end{figure}
...
<<'Fig: registreringer', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
...
<<'Tab: Inklusjoner og oppfølginger', results='asis', echo=FALSE, warning=F>>=
registreringer <- registreringer |> arrange(-(Foer_rapaar+aar_rapaar))
names(registreringer) <- c("Sykehusnavn", paste0("T.o.m. ", rap_aar-1), rap_aar)
oppfolginger <- oppfolginger |> arrange(-(Foer_rapaar+aar_rapaar))
names(oppfolginger) <- c("Sykehusnavn", paste0("T.o.m. ", rap_aar-1), rap_aar)

print(xtable::xtable(
  registreringer, align= c('l', 'l', 'r', 'r'),
  digits = 0,
  caption=paste0("Antall inklusjoner pr. HF")),
  include.rownames = F)

print(xtable::xtable(
  oppfolginger, align= c('l', 'l', 'r', 'r'),
  digits = 0,
  caption=paste0("Antall oppfølginger pr. HF")),
  include.rownames = F)
@
...
\caption{Antall inklusjoner per HF. \\ Filnavn: registreringer.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}reg_pr_diaggr.pdf}
\caption{Antall inklusjoner per HF og per diagnosegruppe. T.o.m. \Sexpr{rap_aar}. \\ Filnavn: reg\_pr\_diaggr.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}reg_pr_diaggr_nysyk.pdf}
\caption{Antall inklusjoner per HF og per diagnosegruppe splittet på nysyke og de som er inkludert med etablert diagnose. T.o.m. \Sexpr{rap_aar}. \\ Filnavn: reg\_pr\_diaggr\_nysyk.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}reg_pr_diagnose.pdf}
\caption{Antall inklusjoner per HF og per diagnose. T.o.m. \Sexpr{rap_aar}. \\ Filnavn: reg\_pr\_diagnose.pdf}
\end{figure}
...
print(xtable::xtable(
  registreringer, align= c('l', 'L{1.5in}', rep('R{0.9in}', dim(registreringer)[2]-1)),
  digits = 0,
...
Medisiner_raa <- Medisiner |>
  dplyr::filter(LegemiddelGenerisk != "Okrelizumab") ## 2026: Fjernet etter ønske
## fra Julianne

## For pasienter som mangler registrering av medisiner leeges det
## til en rad med info om diagnose og inklusjonsdato, samt ## eksklusjonsstatus
ekskluderte <- Oppfolging |>
  dplyr::filter(!is.na(EksklusjonsDato)) |>
  dplyr::select(EksklusjonsArsak, EksklusjonsDato,
                HovedskjemaGUID)
Medisiner <- Medisiner_raa |>
  select(-Diag_gr_nr) |>
  merge(
    Inklusjon[, c("SkjemaGUID", "InklusjonDato",
                  "Diagnose", "Diag_gr", "Diag_gr_nr")],
...
Medisiner[is.na(Medisiner$PasientGUID),
          c("PasientGUID", "Sykehusnavn")] <-
  Inklusjon[match(Medisiner$HovedskjemaGUID[
    is.na(Medisiner$PasientGUID)],
    Inklusjon$SkjemaGUID), c("PasientGUID", "Sykehusnavn")]

Medisiner$Med_SluttDato2 <- Medisiner$Med_SluttDato
Medisiner$Med_SluttDato2[
  is.na(Medisiner$Med_SluttDato) &
    is.na(Medisiner$EksklusjonsDato)] <- as.Date("2100-01-01")
Medisiner$Med_SluttDato2[
  is.na(Medisiner$Med_SluttDato) &
    !is.na(Medisiner$EksklusjonsDato)] <-
  Medisiner$EksklusjonsDato[
    is.na(Medisiner$Med_SluttDato) &
      !is.na(Medisiner$EksklusjonsDato)]
Medisiner$Paa_med <- lubridate::interval(
  Medisiner$Med_StartDato,
  Medisiner$Med_SluttDato2)
rapaar_intervall <- lubridate::interval(
  as.Date(paste0(rap_aar, "-01-01")),
  as.Date(paste0(rap_aar, "-12-31")))
foer_rapaar_intervall <- lubridate::interval(
  as.Date(paste0("1980", "-01-01")),
  as.Date(paste0(rap_aar-1, "-12-31")))
Medisiner$Medisinert_rapaar <- lubridate::int_overlaps(
  Medisiner$Paa_med, rapaar_intervall)
Medisiner$Medisinert_foer_rapaar <- lubridate::int_overlaps(
  Medisiner$Paa_med, foer_rapaar_intervall)
Medisiner$Medisinert_rapaar[
  is.na(Medisiner$Medisinert_rapaar)] <- FALSE
Medisiner$Medisinert_foer_rapaar[
  is.na(Medisiner$Medisinert_foer_rapaar)] <- FALSE
Medisiner$idag <- lubridate::today()
Medisiner$idag[!is.na(Medisiner$EksklusjonsDato)] <-
  Medisiner$EksklusjonsDato[!is.na(Medisiner$EksklusjonsDato)]
Medisiner$inkludert_intervall <- lubridate::interval(
  Medisiner$InklusjonDato,
  Medisiner$idag)
Medisiner$Inkl_rapaar <- lubridate::int_overlaps(
  Medisiner$inkludert_intervall, rapaar_intervall)
Medisiner$Inkl_foer_rapaar <- lubridate::int_overlaps(
  Medisiner$inkludert_intervall, foer_rapaar_intervall)

tabell <- Medisiner |>
  filter(
    Diag_gr_nr == 1,
    Diagnose %in% c(
      "Kjempecellearteritt med polymyalgia rheumatica /Annen kjempecellearteritt", "Uspesifisert arteritt"),
    Inkl_rapaar & Medisinert_rapaar,
    !is.na(Legemiddelgruppe),
    Legemiddelgruppe != "Rituksimab") |>
  summarise(Antall = length(unique(PasientGUID)),
            .by = c(Sykehusnavn, Legemiddelgruppe)) |>
  tidyr::pivot_wider(names_from = Legemiddelgruppe,
                     values_from = Antall, values_fill = 0)
...
plotMatrise <- ant_pas_pr_med_gr1[
  , -c(1,dim(ant_pas_pr_med_gr1)[2])]/ant_pas_pr_med_gr1$N*100
...
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
outfile <- paste0(figfolder, 'medikamentbruk_KCA.', utformat)
# legendTxt <- c("Kortikosteroider", "DMARD", "Biologiske legemidler")
legendTxt <- row.names(plotMatrise)

norvasStabelGrvar_v2(
  plotMatrise=plotMatrise,
  grtxt = grtxt,
  outfile = outfile,
  tittel = c("Medikamentbruk ved KCA og aortitt"),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = T,
  revcol = F,
...
tabell <- Medisiner |>
  filter(
    Diag_gr_nr == 1,
    Diagnose %in% c("Takayasus sykdom (Aortabuesyndrom)"),
    Inkl_rapaar & Medisinert_rapaar,
    !is.na(Legemiddelgruppe),
    Legemiddelgruppe != "Rituksimab") |>
  summarise(Antall = length(unique(PasientGUID)),
...
plotMatrise <- t(as.matrix(plotMatrise))
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
outfile <- paste0(figfolder, 'medikamentbruk_tak.', utformat)
# legendTxt <- c("Kortikosteroider", "DMARD", "Biologiske legemidler")
legendTxt <- row.names(plotMatrise)
...
norvasStabelGrvar_v2(
  ...
  grtxt = grtxt,
  outfile = outfile,
  tittel = c("Medikamentbruk ved Aortabuesyndrom"),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = T,
  revcol = T,
...
ant_pas_pr_med <- Medisiner |>
  ...
  filter(Legemiddelgruppe == "csDMARD") |>
  filter(LegemiddelGenerisk %in% c("Metotreksat", "Leflunomid",
                                   "Azatioprin", "Mycofenolat",
                                   "Mykofenolsyre")) |>
...
ant_pas_pr_med_gr1 <- merge(
  ant_pas_pr_med, N, by = 'Sykehusnavn', all = T)
ant_pas_pr_med_gr1 <- ant_pas_pr_med_gr1[
  ant_pas_pr_med_gr1$N >= 5, ]
...
legendTxt <- norvas::wrap.it(paste0(names(ant_pas_pr_med_gr1)[
  -c(1, dim(ant_pas_pr_med_gr1)[2])], ", N=",
...
norvarStabelGrvar(
  ...
  grtxt = grtxt,
  outfile = outfile,
  tittel = c('Undergrupper av csDMARD,', 'storkarsvaskulitter'),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = F,
  revcol = T,
...
ant_pas_pr_med <- Medisiner |>
  ...
  filter(Legemiddelgruppe %in% c("bDMARD", "tsDMARD")) |>
  group_by(Sykehusnavn, LegemiddelGenerisk) |>
  summarise(Antall = length(unique(PasientGUID))) |>
  ungroup() |>
  spread(key=LegemiddelGenerisk, value = Antall, fill=0)
...
norvarStabelGrvar(
  ...
  grtxt = grtxt,
  outfile = outfile,
  tittel = c('Undergrupper av biologiske',
             'legemidler, storkarsvaskulitter'),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = F,
  revcol = T,
...
tabell <- Medisiner |>
  filter(
    # Diag_gr_nr == 2,
    Diagnose %in% c("Granulomatose med polyangiitt (GPA)",
...
# Resultat
ant_pas_pr_med_gr2 <- merge(
  tabell, N,
  by = 'Sykehusnavn', all = T) |>
  as_tibble()
ant_pas_pr_med_gr2 <- ant_pas_pr_med_gr2[
  ant_pas_pr_med_gr2$N >= 5, ]

plotMatrise <- ant_pas_pr_med_gr2[
  , -c(1, dim(ant_pas_pr_med_gr2)[2])]/ant_pas_pr_med_gr2$N*100
...
outfile <- paste0(figfolder, 'medikamentbruk_mpa_gpa.', utformat)
legendTxt <- row.names(plotMatrise)
# legendTxt <- c("Kortikosteroider", "DMARD", "Syklofosfamid", "Rituksimab")
norvarStabelGrvar(
  plotMatrise=plotMatrise,
  grtxt = grtxt,
  outfile = outfile,
  tittel = c("Medikamentell behandling ved MPA og GPA"),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = T,
  revcol = F,
...
tabell <- Medisiner |>
  filter(
    # Diag_gr_nr == 2,
    Diagnose %in% c("Polyarteritt med lungeaffeksjon (EGPA)")) |>
  filter(Inkl_rapaar & Medisinert_rapaar) |>
  filter(!is.na(Legemiddelgruppe)) |>
  group_by(Sykehusnavn, Legemiddelgruppe) |>
  summarise(Antall = length(unique(PasientGUID))) |>
...
plotMatrise <- t(as.matrix(plotMatrise))
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
outfile <- paste0(figfolder, 'medikamentbruk_egpa.', utformat)
legendTxt <- row.names(plotMatrise)
norvarStabelGrvar(
  plotMatrise=plotMatrise,
  grtxt = grtxt,
  outfile = outfile,
  tittel = c("Medikamentell behandling ved EGPA"),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = T,
  revcol = F,
...
plotvektor <- ant_pas_pr_med_gr2$bDMARD/ant_pas_pr_med_gr2$N*100
...
legendTxt <- ""
norvarStabelGrvar(
  plotMatrise=t(as.matrix(plotvektor)),
  grtxt = grtxt,
  outfile = outfile,
...
ant_pas_pr_med <- Medisiner |>
  ...
  filter(Inkl_rapaar & Medisinert_rapaar) |>
  filter(!is.na(LegemiddelGenerisk)) |>
  filter(LegemiddelGenerisk %in% c("Metotreksat", "Leflunomid", "Azatioprin", "Mycofenolat", "Mykofenolsyre")) |>
  group_by(Sykehusnavn, LegemiddelGenerisk) |>
  summarise(Antall = length(unique(PasientGUID))) |>
  ungroup() |>
  spread(key=LegemiddelGenerisk, value = Antall, fill=0) # |>
...
legendTxt <- norvas::wrap.it(
  paste0(names(ant_pas_pr_med_gr2)[
    -c(1, dim(ant_pas_pr_med_gr2)[2])], ", N=",
    colSums(ant_pas_pr_med_gr2[
      , -c(1, dim(ant_pas_pr_med_gr2)[2])])), 34)
norvarStabelGrvar(
  plotMatrise=plotMatrise,
  grtxt = grtxt,
  outfile = outfile,
...
tabell_med <- Medisiner |>
  # group_by(LegemiddelGenerisk) |>
  summarise(
    med_rapaar = length(unique(
      PasientGUID[Medisinert_rapaar & Inkl_rapaar])),
    med_foer_rapaar = length(unique(
      PasientGUID[Medisinert_foer_rapaar & Inkl_foer_rapaar])),
    .by = LegemiddelGenerisk) |>
  mutate(
    N_rapaar = length(unique(
      Medisiner$PasientGUID[Medisiner$Inkl_rapaar]))) |>
  mutate(
    N_foer_rapaar = length(unique(
      Medisiner$PasientGUID[Medisiner$Inkl_foer_rapaar]))) |>
  filter(!is.na(LegemiddelGenerisk)) |>
...
tabell_medgr <- Medisiner |>
  filter(Legemiddelgruppe %in%
           c("Immunglobulin", "bDMARD", "Syklofosfamid",
             "Rituksimab", "csDMARD", "Glukokortikoid")) |>
  summarise(
    med_rapaar = length(unique(
      PasientGUID[Medisinert_rapaar & Inkl_rapaar])),
    med_foer_rapaar = length(unique(
  ...
  mutate(
    N_rapaar = length(unique(
      Medisiner$PasientGUID[Medisiner$Inkl_rapaar]))) |>
  mutate(N_foer_rapaar = length(unique(
    Medisiner$PasientGUID[Medisiner$Inkl_foer_rapaar]))) |>
  filter(!is.na(Legemiddelgruppe)) |>
  mutate(andel_rapaar = med_rapaar/N_rapaar*100) |>
  mutate(andel_foer_rapaar = med_foer_rapaar/N_foer_rapaar*100) |>
  arrange(andel_foer_rapaar)
plotMatrise <- tabell_medgr |>
  select(andel_rapaar, andel_foer_rapaar) |>
  as.matrix() |> t()
colnames(plotMatrise) <- rep('', dim(plotMatrise)[2])
...
norvarStabelGrvar(
  ...
  grtxt = grtxt,
  outfile = outfile,
  tittel = c("Fordeling av medikamentell behandling på legemiddelgrupper"),
  xlab ="Andel pasienter",
  legendTxt=legendTxt,
  beside = T,
  revcol = T,
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}medikamentbruk_KCA.pdf}
\caption{Utvalg som i Figur \ref{medisin_figur_alle} for \Sexpr{rap_aar}.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}medikamentbruk_tak.pdf}
\caption{Utvalg som i Figur \ref{medisin_figur_alle} for \Sexpr{rap_aar}.}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}medikamentbruk_mpa_gpa.pdf}
\caption{Utvalg som i Figur \ref{medisin_figur_alle} for \Sexpr{rap_aar}.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}medikamentbruk_egpa.pdf}
\caption{Utvalg som i Figur \ref{medisin_figur_alle} for \Sexpr{rap_aar}.}
\end{figure}
...
#             UnitId = first(UnitId),
#             N = sum(Inklusjonsaar < rap_aar & (is.na(EksklusjonsDato) |
...
#     summarise(Sykehusnavn = first(Sykehusnavn),
#               UnitId = first(UnitId),
#               N = sum(Inklusjonsaar < aar & (is.na(EksklusjonsDato) |
#                                                    Eksklusjonsaar > aar)),
...
# Legg til en indikatorvariabel som sier om pasienten har vært følgt opp
Oppfolging$FolgtOpp <- 1

ant_oppf_pr_aar <- merge(
  aux,
...
gjsn_ant_oppf_pr_aar <- ant_oppf_pr_aar |>
  merge(Diagnoser[, c("PasientGUID", "Diag_gr")],
        by = "PasientGUID", all.x = TRUE) |>
  summarise(ant_oppf = mean(Antall_oppf),
            N = n(),
            .by = c(UnitId, Oppfolgingsaar, Diag_gr)) |>
  filter(Oppfolgingsaar == rap_aar) |>
  merge(kobl_unitid_shusnavn_norvas[, c("Sykehusnavn", "UnitId")],
        by = "UnitId", all.x = TRUE) |>
  mutate(verdi = paste0(sprintf("%.1f", ant_oppf), " (", N, ")")) |>
  select(Sykehusnavn, Diag_gr, verdi) |>
  tidyr::pivot_wider(names_from = Diag_gr, values_from = verdi) |>
  arrange(Sykehusnavn) |> select(1,3,2)

print(xtable::xtable(
  gjsn_ant_oppf_pr_aar, align= c('l','l','r','r'),
  caption=paste0('Gjennomsnittlig antall oppfølginger per pasient. Tallene i
                 parantes angir antall pasienter aktuell for oppfølging, dvs.
                 inkludert før ', rap_aar, ' og ikke ekskludert innen ',
                 rap_aar)), include.rownames = F)


indikator_2oppf <- ant_oppf_pr_aar |>
  dplyr::mutate(var = as.numeric(Antall_oppf>=2),
                denominator = 1,
                ind_id = "norvas_minimum_2oppf") |>
  dplyr::rename(year = Oppfolgingsaar) |>
  dplyr::select(UnitId, year, var, denominator, ind_id)
...
# Inklusjon |> dplyr::filter(Diagnose_ny_30 == 1) |>
#   dplyr::mutate(DiagnoseAar = lubridate::year(Diagnose_Klinisk_Dato)) |>
#   dplyr::filter(DiagnoseAar %in% (rap_aar-4):rap_aar) |>
#   dplyr::group_by(Diag_gr, DiagnoseAar) |>
#   dplyr::summarise(Antall = n()) |>
#   tidyr::pivot_wider(names_from = DiagnoseAar, values_from = Antall) |>
#   dplyr::rename(Diagnosegruppe = Diag_gr)
...
terskel <- 5
BVAS_unik <-
  BVAS[!duplicated(BVAS[, c("PasientGUID", "BVAS_Dato")]), ]

########  BVAS ##############
OppfolgingBVAS <- merge(
  Oppfolging,
  BVAS_unik[, c("SykdomsvurderingLabel", "PasientGUID", "BVAS_Dato")],
...
Tabell_bvas_oppf <- OppfolgingBVAS |>
  ...
  dplyr::summarise(
    ...
    N = dplyr::n()) |>
  dplyr::arrange(desc(Antall/N)) |>
  janitor::adorn_totals() |>
  dplyr::mutate(Andel = sprintf("%.1f", Antall/N*100),
                Andel = ifelse(N<terskel, paste0("N<", terskel), Andel)) |>
  dplyr::select(Sykehusnavn, Andel, N)

Tabell_bvas_oppf <- bind_rows(
  Tabell_bvas_oppf |> filter(Andel != paste0("N<", terskel) & Sykehusnavn != "Total"),
  Tabell_bvas_oppf |> filter(Andel == paste0("N<", terskel)),
  Tabell_bvas_oppf |> filter(Sykehusnavn == "Total")) |>
...
Tabell_bvas_oppf_aar <- OppfolgingBVAS |>
  ...
  dplyr::summarise(
    Antall = sum(!is.na(SykdomsvurderingLabel)),
    N = dplyr::n(),
    .by = c(Oppfolgingsaar, Sykehusnavn)) |>
  rename(Aar = Oppfolgingsaar) |>
  group_by(Aar) |>
  group_modify(~ .x |> janitor::adorn_totals()) |>
  # group_by(Sykehusnavn) |>
  # group_modify(~ .x |> janitor::adorn_totals()) |>
  filter(Aar >= rap_aar - 2) |>
...
print(xtable::xtable(
  Tabell_bvas_oppf_aar, align= c('l','l',rep("r", dim(Tabell_bvas_oppf_aar)[2]-1)),
  caption='Andel med utført BVAS ved oppfølging'),
  include.rownames = F)

############# Resultatportalen  ##########################################################

Ind2_AndelBVAS_NorVas <-
  OppfolgingBVAS[, c("UnitId", "Oppfolgingsaar", "SykdomsvurderingLabel")]
Ind2_AndelBVAS_NorVas$Teller <-
  as.numeric(!is.na(Ind2_AndelBVAS_NorVas$SykdomsvurderingLabel))
Ind2_AndelBVAS_NorVas$Nevner <- 1
figurdata <- Ind2_AndelBVAS_NorVas
figurdata$Sykehusnavn <-
  kobl_unitid_shusnavn_norvas$Sykehusnavn[
    match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
@
...
norvasIndikator(
  figurdata[which(figurdata$Oppfolgingsaar==rap_aar),],
  tittel = c("Andel oppfølginger med utfylt BVAS",
             "for ANCA-assosierte vaskulitter"),
  outfile = outfile,
  maal = 95, minstekrav = 50)

outfile <- paste0(figfolder, "bvas_ved_oppf_anca_aar.", utformat)
norvasFigIndikatorAar(
  figurdata |>
    rename(Aar = Oppfolgingsaar) |>
    filter(Aar <= rap_aar),
  tittel = c("Andel oppfølginger med utfylt BVAS",
...
VDI_unik <-
  VDI[!duplicated(VDI[, c("PasientGUID", "VDI_Dato")]), ]

OppfolgingVDI <- merge(
  Oppfolging, VDI_unik[, c("Cataract", "PasientGUID", "VDI_Dato")],
  by.x = c('PasientGUID','OppfolgingsDato'),
  by.y = c('PasientGUID','VDI_Dato'), all.x = T) |>
  filter(Diag_gr_nr %in% 2)

Tabell_vdi_oppf <- OppfolgingVDI |>
  dplyr::filter(Oppfolgingsaar==rap_aar) |>
  dplyr::group_by(Sykehusnavn) |>
  dplyr::summarise(
    Antall = sum(!is.na(Cataract)),
    N = dplyr::n()) |>
  dplyr::arrange(desc(Antall/N)) |>
  janitor::adorn_totals() |>
  dplyr::mutate(Andel = sprintf("%.1f", Antall/N*100),
...
Tabell_vdi_oppf <- bind_rows(
  Tabell_vdi_oppf |> filter(Andel != paste0("N<", terskel) & Sykehusnavn != "Total"),
  Tabell_vdi_oppf |> filter(Andel == paste0("N<", terskel)),
  Tabell_vdi_oppf |> filter(Sykehusnavn == "Total")) |>
  dplyr::rename("Andel oppfølging" = "Andel")


print(xtable::xtable(Tabell_vdi_oppf, align= c('l','l','r','r'), digits=c(0,0,1,0), caption=paste0('Andel oppfølgingskjema med utfylt VDI-skjema i ', rap_aar, ', ANCA-assosierte vaskulitter')), include.rownames = F)

############# Resultatportalen  ##########################################################

AndelVDI_NorVas <- OppfolgingVDI[, c("UnitId", "Oppfolgingsaar", "Cataract")]
AndelVDI_NorVas$Teller <- as.numeric(!is.na(AndelVDI_NorVas$Cataract))
AndelVDI_NorVas$Nevner <- 1
figurdata <- AndelVDI_NorVas
figurdata$Sykehusnavn <- kobl_unitid_shusnavn_norvas$Sykehusnavn[match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]

tab_oppf_vdi <- figurdata |>
  rename(Aar = Oppfolgingsaar) |>
  filter(Aar <= rap_aar) |>
  dplyr::summarise(
    Antall = sum(!is.na(Cataract)),
...
print(xtable::xtable(
  tab_oppf_vdi, align= c('l','l',rep("r", dim(tab_oppf_vdi)[2]-1)),
  caption='Andel oppfølgingskjema med utfylt VDI-skjema for ANCA-assosierte vaskulitter'),
  include.rownames = F)
...
norvasIndikator(figurdata[which(figurdata$Oppfolgingsaar==rap_aar),],
                tittel = c("Andel oppfølginger med utfylt VDI",
                           "for ANCA-assosierte vaskulitter"),
                outfile = outfile, maal = 95, minstekrav = 50)

outfile <- paste0(figfolder, "vdi_ved_oppf_anca_aar.", utformat)
norvasFigIndikatorAar(figurdata |>
                        rename(Aar = Oppfolgingsaar) |>
                        filter(Aar <= rap_aar),
                      tittel = c("Andel oppfølginger med utfylt VDI",
...
Ind2_AndelBVAS_NorVas <- Ind2_AndelBVAS_NorVas[ , -3]

Ind2_AndelBVAS_NorVas$ind_id <- "norvas_bvas"

Indikatorer <- Ind2_AndelBVAS_NorVas |>
...
KERR_unik <-
  KERR[!duplicated(KERR[, c("PasientGUID", "KerrsKriterier_Dato")]), ]

OppfolgingKERR <- merge(
  Oppfolging,
  KERR_unik[, c("KerrsKriterier_Dato", "PasientGUID", "Sykdomsvurdering")],
  by.x = c('PasientGUID','OppfolgingsDato'),
  by.y = c('PasientGUID','KerrsKriterier_Dato'), all.x = T) |>
  dplyr::filter(Diag_gr_nr == 1)


Tabell_kerr_oppf <- OppfolgingKERR |>
  dplyr::filter(Oppfolgingsaar==rap_aar) |>
  dplyr::group_by(Sykehusnavn) |>
  dplyr::summarise(
    Antall = sum(!is.na(Sykdomsvurdering)),
    N = dplyr::n()) |>
  dplyr::arrange(desc(Antall/N)) |>
  janitor::adorn_totals() |>
  dplyr::mutate(Andel = sprintf("%.1f", Antall/N*100),
...
Tabell_kerr_oppf <- bind_rows(
  Tabell_kerr_oppf |> filter(Andel != paste0("N<", terskel) &
                               Sykehusnavn != "Total"),
  Tabell_kerr_oppf |> filter(Andel == paste0("N<", terskel)),
  Tabell_kerr_oppf |> filter(Sykehusnavn == "Total")) |>
  dplyr::rename("Andel oppfølging" = "Andel")


print(xtable::xtable(
  Tabell_kerr_oppf, align= c('l','l','r','r'),
  digits=c(0,0,1,0),
  caption=paste0('Andel med utført Kerrs ved oppfølging i ',
                 rap_aar, ', gr. 1')), include.rownames = F)



Tabell_kerr_oppf_aar <- OppfolgingKERR |>
  dplyr::filter(Oppfolgingsaar<=rap_aar) |>
  dplyr::summarise(
    Antall = sum(!is.na(Sykdomsvurdering)),
    N = dplyr::n(),
...
print(xtable::xtable(
  Tabell_kerr_oppf_aar, align= c('l','l',rep("r", dim(Tabell_kerr_oppf_aar)[2]-1)),
  caption='Andel med utført KERRS ved oppfølging'),
  include.rownames = F)
...
ind3_AndelKerr_NorVas <-
  OppfolgingKERR[, c("UnitId", "Oppfolgingsaar", "Sykdomsvurdering")]
ind3_AndelKerr_NorVas$Teller <-
  as.numeric(!is.na(ind3_AndelKerr_NorVas$Sykdomsvurdering))
ind3_AndelKerr_NorVas$Nevner <- 1
figurdata <- ind3_AndelKerr_NorVas
figurdata$Sykehusnavn <-
  kobl_unitid_shusnavn_norvas$Sykehusnavn[
    match(figurdata$UnitId, kobl_unitid_shusnavn_norvas$UnitId)]
@
...
norvasIndikator(
  figurdata[which(figurdata$Oppfolgingsaar==rap_aar),],
  tittel = c("Andel med utført Kerrs ved oppfølging",
             "for storkarsvaskulitter"),
  maal = 80, minstekrav = 40, outfile = outfile)

outfile <- paste0(figfolder, "kerr_ved_oppf_gvv_aar.", utformat)
norvasFigIndikatorAar(figurdata |>
                        rename(Aar = Oppfolgingsaar) |>
                        filter(Aar <= rap_aar),
                      tittel = c("Andel med utført Kerrs ved oppfølging",
...
ind3_AndelKerr_NorVas <- ind3_AndelKerr_NorVas[ , -3]

ind3_AndelKerr_NorVas$ind_id <- "norvas_kerr"

Indikatorer <- ind3_AndelKerr_NorVas |>
  rename(year = Oppfolgingsaar,
         var = Teller,
         denominator = Nevner) |>
  select(UnitId, year, var, denominator, ind_id) |>
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}bvas_ved_oppf_anca_aar.pdf}
\caption{}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}vdi_ved_oppf_anca_aar.pdf}
\caption{}
\end{figure}
...
print(xtable::xtable(
  ...
  caption='Andel ANCA test ved debut av ANCA-assosiert vaskulitt etter inklusjonsår.\\ Nevner: Pasienter med ANCA-assosiert vaskulitt inkludert gjeldende år. Teller: Pasienter som har et labskjema tilknyttet inklusjonsskjema som har utfylt enten PR3AncaPositiv eller MPO\\_AncaPositiv',
  include.rownames = F))
...
norvasIndikator(figurdata,
                tittel = c("Andel med utført ANCA-test ved debut",
                           "for ANCA-assosierte vaskulitter"),
                maal = 95, minstekrav = 50, outfile = outfile, xmax = 100)
outfile <- paste0(figfolder, "anca_v_debut_ancaass_vas_aar.", utformat)
norvasFigIndikatorAar(figurdata |> rename(Aar = Inklusjonsaar),
                      tittel = c("Andel med utført ANCA-test ved debut",
                                 "for ANCA-assosierte vaskulitter"),
                      maal = 95, minstekrav = 50, outfile = outfile, xmax = 100)
...
\caption{Andel ANCA test ved debut av ANCA-assosiert vaskulitt.\\ Nevner: Pasienter med ANCA-assosiert vaskulitt. Teller: Pasienter som har et labskjema tilknyttet inklusjonsskjema som har utfylt enten PR3AncaPositiv eller MPO\_AncaPositiv. Gjelder for pasienter inkludert t.o.m. \Sexpr{rap_aar}.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}anca_v_debut_ancaass_vas_aar.pdf}
\caption{Andel ANCA test ved debut av ANCA-assosiert vaskulitt.\\ Nevner: Pasienter med ANCA-assosiert vaskulitt. Teller: Pasienter som har et labskjema tilknyttet inklusjonsskjema som har utfylt enten PR3AncaPositiv eller MPO\_AncaPositiv. Gjelder for pasienter inkludert fra \Sexpr{rap_aar-2} til \Sexpr{rap_aar}.}
\end{figure}
...
BVAS$tid_diag_bvas <- difftime(BVAS$BVAS_Dato,
                               BVAS$Diagnose_Klinisk_Dato, units = 'days')

InklusjonBVAS <- Inklusjon |>
  select(Diagnose_ny_30,InklusjonDato,
         SkjemaGUID, Diag_gr_nr, Sykehusnavn,
         UnitId, PasientGUID, Diagnose_Klinisk_Dato) |>
  filter(Diag_gr_nr==2) |>
  merge(BVAS[, c("HovedskjemaGUID", "BVAS_Dato",
                 "tid_diag_bvas", "Sykdomsvurdering")],
        by.x = c("SkjemaGUID"),
        by.y = c("HovedskjemaGUID"),
        all.x = TRUE) |>
  filter(BVAS_Dato >= Diagnose_Klinisk_Dato + months(6) -
           lubridate::weeks(4),
         BVAS_Dato <= Diagnose_Klinisk_Dato + months(6) +
           lubridate::weeks(4)) |>
  mutate(Aar = lubridate::year(Diagnose_Klinisk_Dato)) |>
  summarise(Teller = as.numeric(5 %in% Sykdomsvurdering),
            N = n(),
            Sykehusnavn = first(Sykehusnavn),
            .by = c(PasientGUID, Aar, UnitId)) |>
  mutate(Aar <= rap_aar) |>
  mutate(Nevner = 1,
         ind_id = "norvas_anca_remisjon")

ind4_AndelAnCA_remisjon_NorVas <- InklusjonBVAS |>
  select(UnitId, Aar, Teller, Sykehusnavn, Nevner, ind_id)


Indikatorer <- ind4_AndelAnCA_remisjon_NorVas |>
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  select(UnitId, year, var, denominator, ind_id) |>
...
tabell_remisjon_6mnd <- ind4_AndelAnCA_remisjon_NorVas |>
  filter(Aar <= rap_aar) |>
  group_by(Sykehusnavn) |>
...
tabell_remisjon_6mnd_aar <- ind4_AndelAnCA_remisjon_NorVas |>
  filter(Aar <= rap_aar) |>
  summarise(Antall = sum(Teller),
            N = n(),
            .by = c(Sykehusnavn, Aar)) |>
  ...
  mutate(Andel = Antall/N*100,
         ...
         Andel = ifelse(N<5, "N<5", Andel)) |>
  select(Sykehusnavn, Aar, Andel) |>
  filter(Aar > rap_aar-5) |>
  tidyr::pivot_wider(names_from = Aar, values_from = Andel, names_sort = TRUE) |>
  arrange(Sykehusnavn)


print(xtable::xtable(tabell_remisjon_6mnd, align= c('l','l','r','r'), digits=c(0,0,1,0), caption=paste0('Andel i remisjon innen 6 måneder fra diagnosedato ut fra BVAS for ANCA-assosiert vaskulitter t.o.m. ', rap_aar, '. Kun pasienter som har utfylt BVAS ved 6 mnd. plusminus 4 uker etter diagnosedato inngår.')), include.rownames = F)

print(xtable::xtable(tabell_remisjon_6mnd_aar, align= c('l','l',rep("r", dim(tabell_remisjon_6mnd_aar)[2]-1)), caption= 'Andel i remisjon innen 6 måneder fra diagnosedato ut fra BVAS for ANCA-assosiert vaskulitter. Kun pasienter som har utfylt BVAS ved 6 mnd. plusminus 4 uker etter diagnosedato inngår.'), include.rownames = F)


@

<<'Fig. anca remisjon v2', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
figurdata <- ind4_AndelAnCA_remisjon_NorVas
outfile <- paste0(figfolder, "remisjon_6mnd_anca.", utformat)
norvasIndikator(
  figurdata[which(figurdata$Aar<=rap_aar),],
  tittel = c("Andel i remisjon innen 6 måneder ut fra",
             "BVAS for ANCA-assosierte vaskulitter"),
  maal = 80, minstekrav = 40, outfile = outfile, xmax = 100)

outfile <- paste0(figfolder, "remisjon_6mnd_anca_aar.", utformat)
norvasFigIndikatorAar(
  figurdata[which(figurdata$Aar<=rap_aar),],
  tittel = c("Andel i remisjon innen 6 måneder ut fra",
             "BVAS for ANCA-assosierte vaskulitter"),
...
\caption{Andel pasienter med ANCA-assosiert vaskulitt som er i remisjon 6 mndr etter debut. Nevner: Alle med ANCA-assosiert vaskulitt som har et utfylt BVAS-skjema 6 mnd $\pm$ 4 uker etter diagnosedato. Gjelder alle med diagnosedato t.o.m. \Sexpr{rap_aar}}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}remisjon_6mnd_anca_aar.pdf}
\caption{Andel pasienter med ANCA-assosiert vaskulitt som er i remisjon 6 mndr, per år.}
\end{figure}

<<'Tab: andel remisjon 6mnd, GVV, V2', results='asis', echo=FALSE, warning=F>>=
KERR$tid_diag_kerr <- difftime(KERR$KerrsKriterier_Dato,
                               KERR$Diagnose_Klinisk_Dato, units = 'days')

InklusjonKERR <- Inklusjon |>
  select(Diagnose_ny_30,InklusjonDato,
         SkjemaGUID, Diag_gr_nr, Sykehusnavn,
         UnitId, PasientGUID, Diagnose_Klinisk_Dato) |>
  filter(Diag_gr_nr==1) |>
  merge(KERR[, c("HovedskjemaGUID", "KerrsKriterier_Dato",
                 "tid_diag_kerr", "Sykdomsvurdering")],
        by.x = c("SkjemaGUID"),
        by.y = c("HovedskjemaGUID"),
        all.x = TRUE) |>
  filter(KerrsKriterier_Dato >= Diagnose_Klinisk_Dato + months(6) -
           lubridate::weeks(4),
         KerrsKriterier_Dato <= Diagnose_Klinisk_Dato + months(6) +
           lubridate::weeks(4)) |>
  mutate(Aar = lubridate::year(Diagnose_Klinisk_Dato)) |>
  summarise(Teller = as.numeric(2 %in% Sykdomsvurdering),
            N = n(),
            Sykehusnavn = first(Sykehusnavn),
            .by = c(PasientGUID, Aar, UnitId)) |>
  mutate(Aar <= rap_aar) |>
  mutate(Nevner = 1,
         ind_id = "norvas_storkar_remisjon")

ind6_AndelStorkar_remisjon_NorVas <- InklusjonKERR |>
  select(UnitId, Aar, Teller, Sykehusnavn, Nevner, ind_id)

Indikatorer <- ind6_AndelStorkar_remisjon_NorVas |>
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  select(UnitId, year, var, denominator, ind_id) |>
...
tabell_remisjon_6mnd <- ind6_AndelStorkar_remisjon_NorVas |>
  filter(Aar <= rap_aar) |>
  group_by(Sykehusnavn) |>
  summarise(Antall = sum(Teller),
            N = n()) |>
...
tabell_remisjon_6mnd_aar <- ind6_AndelStorkar_remisjon_NorVas |>
  filter(Aar <= rap_aar,
         Aar > rap_aar-5) |>
  summarise(Antall = sum(Teller),
            N = n(),
...
# tabell_remisjon_6mnd_aar_raa <- ind6_AndelStorkar_remisjon_NorVas |>
#   filter(Aar <= rap_aar) |>
#   summarise(Antall = sum(Teller),
#             N = n(),
#             .by = c(Sykehusnavn, Aar)) |>
...
#   group_by(Sykehusnavn) |>
#   group_modify(~ .x |> janitor::adorn_totals()) |>
#   filter(Aar > rap_aar-3) |>
#   tidyr::pivot_wider(names_from = Aar, values_from = c(Antall, N), names_sort = TRUE)

print(xtable::xtable(
  tabell_remisjon_6mnd, align= c('l','l','r','r'),
  digits=c(0,0,1,0),
  caption=paste0(
    'Andel i remisjon innen 6 måneder fra diagnosedato ut fra KERR
     for storkarsvaskulitter t.o.m. ', rap_aar, '. Kun pasienter som har utfylt
    KERR ved 6 mnd. plusminus 4 uker etter diagnosedato inngår.')),
  include.rownames = F)

print(xtable::xtable(
  tabell_remisjon_6mnd_aar,
  align= c('l','l',rep("r", dim(tabell_remisjon_6mnd_aar)[2]-1)),
  caption= 'Andel i remisjon innen 6 måneder fra diagnosedato ut fra KERR for
  storkarsvaskulitter. Kun pasienter som har utfylt KERR ved 6 mnd.
  plusminus 4 uker etter diagnosedato inngår.'),
  include.rownames = F)
...
figurdata <- ind6_AndelStorkar_remisjon_NorVas
outfile <- paste0(figfolder, "remisjon_6mnd_storkar_v2.", utformat)
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),],
                tittel = c("Andel i remisjon innen 6 måneder ut fra",
                           "Kerrs for storkarsvaskulitter"),
                maal = 80, minstekrav = 40, outfile = outfile)
outfile <- paste0(figfolder, "remisjon_6mnd_storkar_aar.", utformat)
norvasFigIndikatorAar(figurdata[which(figurdata$Aar<=rap_aar),],
                      tittel = c("Andel i remisjon innen 6 måneder ut fra",
                                 "Kerrs for storkarsvaskulitter"),
                      maal = 80, minstekrav = 40, outfile = outfile)
...
\caption{Andel pasienter med storkarsvaskulitt som er i remisjon 6 mndr etter debut, basert på Kerrs. Gjelder alle diagnosedato t.o.m. \Sexpr{rap_aar}}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}remisjon_6mnd_storkar_aar.pdf}
\caption{Andel pasienter med storkarsvaskulitt som er i remisjon 6 mndr, per år.}
\end{figure}

<<'Tab. andel pred.dose mindre enn x', results='asis', echo=FALSE, warning=F>>=

############### Andel på prednisolondose mindre enn 5 etter 6 mnd  ######

prednisolon_6mnd_v2 <- Inklusjon |>
  select(Diagnose_ny_30,InklusjonDato, SkjemaGUID,
         Diag_gr_nr, Sykehusnavn, UnitId, PasientGUID,
         Diagnose_Klinisk_Dato) |>
  filter(Diag_gr_nr==2) |>
  mutate(
    Ved_start_int = lubridate::interval(
      Diagnose_Klinisk_Dato - lubridate::weeks(4),
      Diagnose_Klinisk_Dato + lubridate::weeks(4)),
    Ved_6mnd_int = lubridate::interval(
      Diagnose_Klinisk_Dato %m+% months(6) - lubridate::weeks(4),
      Diagnose_Klinisk_Dato %m+% months(6) + lubridate::weeks(4))) |>
  merge(Medisiner |> filter(LegemiddelGenerisk == "Prednisolon") |>
          select(HovedskjemaGUID, Med_StartDato,
                 Med_SluttDato, Dose),
        by.x = "SkjemaGUID", by.y = "HovedskjemaGUID") |>
  dplyr::filter(Med_StartDato %within% Ved_6mnd_int) |>
  dplyr::filter(abs(difftime(Med_StartDato,
                             Diagnose_Klinisk_Dato %m+% months(6))) ==
                  min(abs(difftime(Med_StartDato,
                                   Diagnose_Klinisk_Dato %m+% months(6)))),
                .by = PasientGUID)|>
  mutate(Teller = as.numeric(Dose <= 5),
         Nevner = 1,
         Aar = lubridate::year(Diagnose_Klinisk_Dato),
         ind_id = "norvas_anca_prednisolon")


Tabell_v2 <- prednisolon_6mnd_v2 |>
  summarise(Antall = sum(Teller),
            N = dplyr::n(),
            .by = c(Sykehusnavn, Aar)) |>
  group_by(Aar) |>
...
print(xtable::xtable(
  Tabell_v2,
  align= c('l','l',rep("r", dim(Tabell_v2)[2]-1)),
  caption= 'Andel pasienter med ANCA-assosiert vaskulitt på
  Prednisolon $\\leq$ 5mg 6 måneder etter debut'),
  include.rownames = F)
...
oppf_6mnd <- Inklusjon |>
  ...
  mutate(
    Ved_6mnd_int = lubridate::interval(
      Diagnose_Klinisk_Dato %m+% months(6) - lubridate::weeks(4),
      Diagnose_Klinisk_Dato %m+% months(6) + lubridate::weeks(4))) |>
  merge(Oppfolging |> select(HovedskjemaGUID, OppfolgingsDato),
        by.x = "SkjemaGUID", by.y = "HovedskjemaGUID",
        all.x = TRUE) |>
  mutate(oppf6mnd = OppfolgingsDato %within% Ved_6mnd_int |>
           as.numeric(),
         oppf6mnd = ifelse(is.na(oppf6mnd), 0, oppf6mnd),
         Aar = lubridate::year(Diagnose_Klinisk_Dato)) |>
  summarise(fulgt_opp = max(oppf6mnd),
            Sykehusnavn = first(Sykehusnavn),
            .by = c(PasientGUID, UnitId, Aar)) |>
  summarise(Antall = sum(fulgt_opp),
            N = n(),
            .by = c(Sykehusnavn, Aar)) |>
  group_by(Aar) |>
  group_modify(~ .x |> janitor::adorn_totals()) |>
...
print(xtable::xtable(
  oppf_6mnd,
  align= c('l','l',rep("r", dim(oppf_6mnd)[2]-1)),
  caption= 'Andel pasienter med ANCA med utfylt oppfølgingsskjema
  6mnd. plusminus 4 uker etter diagnosedato'),
  include.rownames = F)
...
Indikatorer <- prednisolon_6mnd_v2 |>
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  select(UnitId, year, var, denominator, ind_id) |>
...
<<'Fig: anca predni', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
outfile <- paste0(figfolder, "lav_prednisolon_anca_v2.", utformat)
norvasIndikator(prednisolon_6mnd_v2 |> filter(Aar<=rap_aar),
                tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
                           "på Prednisolon <= 5mg 6 måneder etter debut"),
                maal = 60, minstekrav = 30, outfile = outfile, xmax = 100)

@


<<'Tab: andel pred.dose mindre enn 7.5, KERR:Storkarsvaskulitter', results='asis', echo=FALSE, warning=F>>=
...
prednisolon_6mnd_v2 <- Inklusjon |>
  ...
  merge(Medisiner |> filter(LegemiddelGenerisk == "Prednisolon") |>
          select(HovedskjemaGUID, Med_StartDato,
                 Med_SluttDato, Dose),
        by.x = "SkjemaGUID", by.y = "HovedskjemaGUID") |>
  filter(Med_StartDato %within% Ved_6mnd_int) |>
  dplyr::filter(abs(difftime(Med_StartDato,
                             Diagnose_Klinisk_Dato %m+% months(6))) ==
                  min(abs(difftime(Med_StartDato,
                                   Diagnose_Klinisk_Dato %m+% months(6)))),
                .by = PasientGUID)|>
  mutate(Teller = as.numeric(Dose <= 7.5),
         Nevner = 1,
         Aar = lubridate::year(Diagnose_Klinisk_Dato),
         ind_id = "norvas_storvas_prednisolon")
...
print(xtable::xtable(
  Tabell_v2,
  align= c('l','l',rep("r", dim(Tabell_v2)[2]-1)),
  caption= 'Andel pasienter med storkarsvaskulitt på
  Prednisolon $\\leq$ 7.5mg 6 måneder etter debut'),
  include.rownames = F)
...
print(xtable::xtable(
  oppf_6mnd,
  align= c('l','l',rep("r", dim(oppf_6mnd)[2]-1)),
  caption= 'Andel pasienter med storkarsvaskulitt med utfylt oppfølgingsskjema
  6mnd. plusminus 4 uker etter diagnosedato'),
  include.rownames = F)
...
<<'Fig: storkar predni', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=

outfile <- paste0(figfolder, "lav_prednisolon_gvv_v2.", utformat)
norvasIndikator(prednisolon_6mnd_v2 |> filter(Aar<=rap_aar),
                tittel = c("Andel pasienter med storkarsvaskulitt",
                           "på Prednisolon <= 7.5mg 6 måneder etter debut"),
                maal = 60, minstekrav = 30, outfile = outfile, xmax = 100)
@
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}lav_prednisolon_anca_v2.pdf}
\caption{Andel pasienter med ANCA-assosiert vaskulitt på Prednisolon $\leq$ 5mg 6 måneder etter debut. Gjelder alle år t.o.m. \Sexpr{rap_aar}.  \\ Utvalg: Pasienter med ANCA-assosiert vaskulitt som har en registrering av prednisolon 6 måneder $\pm$ 4 uker etter diagnosedato. }
\end{figure}

% \begin{figure}[ht]
% \centering
% \includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}lav_prednisolon_anca_v1.pdf}
% \caption{Andel pasienter med ANCA-assosiert vaskulitt på Prednisolon $\leq$ 5mg 6 måneder etter debut. Gjelder alle år t.o.m. \Sexpr{rap_aar}.  \\ Utvalg: Pasienter med ANCA-assosiert vaskulitt som har en registrering av prednisolon ved diagnosedato $\pm$ 4 uker samt etter 6 måneder $\pm$ 4 uker etter diagnosedato.}
% \end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}lav_prednisolon_gvv_v2.pdf}
\caption{Andel pasienter med storkarsvaskulitt på Prednisolon $\leq$ 7.5mg 6
måneder etter debut. Gjelder alle år t.o.m. \Sexpr{rap_aar}. \\ Utvalg:
Pasienter som har en registrering av prednisolon $\pm$ 4 uker av 6 måneder
etter diagnosedato. }
\end{figure}

% \begin{figure}[ht]
% \centering
% \includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}lav_prednisolon_gvv_v1.pdf}
% \caption{Andel pasienter med storkarsvaskulitt på Prednisolon $\leq$ 7.5mg 6 måneder
% etter debut. Gjelder alle år t.o.m. \Sexpr{rap_aar}. \\ Utvalg: Pasienter som har en registrering av prednisolon ved diagnosedato $\pm$ 4 uker samt registrering av prednisolon $\pm$ 4 uker av 6 måneder etter diagnosedato. }
% \end{figure}
...
indikator_residiv_bvas <- BVAS |>
  ...
  summarise(UnitId = first(UnitId),
            ...
            .by = c(Sykehusnavn, BVAS_aar, PasientGUID)) |>
  mutate(Nevner = 1) |>
  rename(Aar = BVAS_aar)

Tabell_residiv_bvas <- indikator_residiv_bvas |>
  summarise(Antall = sum(Teller),
            N = n(),
            .by = c(Sykehusnavn, Aar)) |>
  group_by(Aar) |>
...
indikator_residiv_kerr <- KERR |>
  ...
  summarise(UnitId = first(UnitId),
            ...
            .by = c(Sykehusnavn, KERR_aar, PasientGUID)) |>
  mutate(Nevner = 1) |>
  rename(Aar = KERR_aar)

Tabell_residiv_kerr <- indikator_residiv_kerr |>
  summarise(Antall = sum(Teller),
            N = n(),
            .by = c(Sykehusnavn, Aar)) |>
  group_by(Aar) |>
...
print(xtable::xtable(
  Tabell_residiv_bvas,
  align= c('l','l',rep("r", dim(Tabell_residiv_bvas)[2]-1)),
  caption= 'Andel pasienter med ANCA-assosiert vaskulitt med lett eller
  alvorlig residiv'),
  include.rownames = F)

print(xtable::xtable(
  Tabell_residiv_kerr,
  align= c('l','l',rep("r", dim(Tabell_residiv_kerr)[2]-1)),
  caption= 'Andel pasienter med storkarsvaskulitt med lett eller
  alvorlig residiv'),
  include.rownames = F)
...
norvasIndikator(indikator_residiv_bvas |> filter(Aar == rap_aar, !is.na(Sykehusnavn)),
                tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
                           "med lett eller alvorlig residiv"),
                outfile = outfile)
outfile <- paste0(figfolder, "Andel_residiv_bvas_tidsfig.", utformat)
norvasFigIndikator(indikator_residiv_bvas |> filter(Aar <= rap_aar, !is.na(Sykehusnavn)),
                   tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
                              "med lett eller alvorlig residiv"),
                   outfile = outfile)
outfile <- paste0(figfolder, "Andel_residiv_bvas_alletider.", utformat)
norvasIndikator(residiv_bvas_alletider |> filter(!is.na(Sykehusnavn)),
                tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
                           "med lett eller alvorlig residiv"),
                outfile = outfile)

outfile <- paste0(figfolder, "Andel_residiv_kerr_rapaar.", utformat)
norvasIndikator(indikator_residiv_kerr |> filter(Aar == rap_aar, !is.na(Sykehusnavn)),
                tittel = c("Andel pasienter med storkarsvaskulitt",
                           "med residiv, basert på Kerrs"),
                outfile = outfile)
outfile <- paste0(figfolder, "Andel_residiv_kerr_tidsfig.", utformat)
norvasFigIndikator(indikator_residiv_kerr |> filter(Aar <= rap_aar, !is.na(Sykehusnavn)),
                   tittel = c("Andel pasienter med storkarsvaskulitt",
                              "med residiv, basert på Kerrs"),
                   outfile = outfile)
outfile <- paste0(figfolder, "Andel_residiv_kerr_alletider.", utformat)
norvasIndikator(residiv_kerr_alletider |> filter(!is.na(Sykehusnavn)),
                tittel = c("Andel pasienter med storkarsvaskulitt",
                           "med residiv, basert på Kerrs"),
                outfile = outfile)
...
Indikatorer <-
  ...
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  mutate(ind_id = "norvas_andel_ct_thorax_anca") |>
  select(UnitId, year, var, denominator, ind_id) |>
  bind_rows(Indikatorer)
...
Indikatorer <-
  ...
  rename(year = Aar,
         var = ctmr_bihuler,
         denominator = Nevner) |>
  mutate(ind_id = "norvas_andel_ctmr_bihuler_anca") |>
  select(UnitId, year, var, denominator, ind_id) |>
  bind_rows(Indikatorer)
...
norvasIndikator(figurdata[which(figurdata$Aar<=rap_aar),] |> dplyr::filter(!is.na(Sykehusnavn)),
...
Indikatorer <-
  ...
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  mutate(ind_id = "norvas_andel_ulctmr_storkar") |>
  select(UnitId, year, var, denominator, ind_id) |>
  bind_rows(Indikatorer)
...
aux <- merge(nysyke, Utredning, by = 'SkjemaGUID', by.y = 'HovedskjemaGUID', all.x = T)
aux <- aux[aux$Diagnose == "Kjempecellearteritt med polymyalgia rheumatica /Annen kjempecellearteritt", ]
...
Indikatorer <-
  ...
  rename(year = Aar,
         var = Teller,
         denominator = Nevner) |>
  mutate(ind_id = "norvas_andel_utredet_GCA") |>
  select(UnitId, year, var, denominator, ind_id) |>
  bind_rows(Indikatorer)
...
# Inklusjon |>
#   filter(Diagnose_ny_30 == 1,
#          FormDate >= paste0(rap_aar-1, "-01-01"),
#          FormDate <= datoTil)
#
...
nysyke2 <- Pasientsvar[, c("PasientGUID", "FormDate", "SkjemaGUID",
                           "Prom_Tretthet",
                           "Prom_PasientGlobalSykdomsaktivitet",
                           "Prom_PasientSmerter")] |>
  filter(!is.na(FormDate)) |>
  merge(nysyke[, c("PasientGUID", "InklusjonDato", "Inklusjonsaar",
                   "Tretthet", "PasientGlobalSykdomsaktivitet",
                   "Pasientsmerter", "Sykehusnavn")],
        by = "PasientGUID", suffixes = c("_pas_svar", ""),
        all.y = TRUE) |>
  mutate(tid_inkl_svar = abs(difftime(InklusjonDato, FormDate, units = "days"))) |>
  mutate(tid_inkl_svar = ifelse(is.na(tid_inkl_svar), 100000, tid_inkl_svar)) |>
  filter(tid_inkl_svar == min(tid_inkl_svar),.by = "PasientGUID") |>
  mutate(
    Tretthet = ifelse(is.na(Tretthet) & tid_inkl_svar <= 30,
                      Prom_Tretthet, Tretthet),
    PasientGlobalSykdomsaktivitet = ifelse(is.na(PasientGlobalSykdomsaktivitet) &
                                             tid_inkl_svar <= 30,
                                           Prom_PasientGlobalSykdomsaktivitet,
                                           PasientGlobalSykdomsaktivitet),
    Pasientsmerter = ifelse(is.na(Pasientsmerter) & tid_inkl_svar <= 30,
                            Prom_PasientSmerter, Pasientsmerter))

## Utvalg nysyke: Diagnose_ny_30 == 1, enten utfylt at inkludert nysyk, eller
## inklusjonsdato og diagnosedato innefor plusminus 30 dager. Med utgangspunkt i
## Pasientsvar-skjema, koble inn nysyke. Per pasient, behold registrering med
## kortest tid mellom


Oppfolging_rap_aar <- Pasientsvar[, c("PasientGUID", "FormDate", "SkjemaGUID",
                                      "Prom_Tretthet",
                                      "Prom_PasientGlobalSykdomsaktivitet",
                                      "Prom_PasientSmerter", "Sykehusnavn")] |>
  filter(!(SkjemaGUID %in% nysyke$SkjemaGUID[nysyke$tid_inkl_svar <= 30])) |>
  select(-SkjemaGUID) |>
  rename(OppfolgingsDato=FormDate,
         Tretthet = Prom_Tretthet,
         PasientGlobalSykdomsaktivitet = Prom_PasientGlobalSykdomsaktivitet,
         Pasientsmerter = Prom_PasientSmerter) |>
  mutate(Oppfolgingsaar = format(OppfolgingsDato, "%Y") |> as.numeric()) |>
  filter(!is.na(OppfolgingsDato)) |>
  bind_rows(Oppfolging[, c("PasientGUID", "OppfolgingsDato",
                           "Oppfolgingsaar", "Tretthet", "Sykehusnavn",
                           "PasientGlobalSykdomsaktivitet", "Pasientsmerter")]) |>
  filter(PasientGUID %in% nysyke$PasientGUID,
...
tretthet_pre_post <-
  merge(prom_inklusjon[, c("Sykehusnavn", "gjsn.tretthet", "n1")],
        prom_oppf[, c("Sykehusnavn", "gjsn.tretthet", "n1")],
        suffixes = c("_pre", "_post"), by = "Sykehusnavn")
pasientglobalsykdomsaktivitet_pre_post <-
  merge(prom_inklusjon[, c("Sykehusnavn", "gjsn.pasientglobalsykdomsaktivitet", "n2")],
        prom_oppf[, c("Sykehusnavn", "gjsn.pasientglobalsykdomsaktivitet", "n2")],
        suffixes = c("_pre", "_post"), by = "Sykehusnavn")
pasientsmerter_pre_post <-
  merge(prom_inklusjon[, c("Sykehusnavn", "gjsn.pasientsmerter", "n3")],
...
\caption{Gjennomsnittlig pasientrapportert tretthet ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}. Nysyke er enten registrert direkte som nysyke eller, hvis nysykvariabelen er tom, har 30 dager eller mindre mellom diagnosedato og inklusjonsdato. Én pasient kan ha flere oppfølginger.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}globalsykdomsaktivitet.pdf}
\caption{Gjennomsnittlig pasientrapportert global sykdomsaktivitet ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}. Nysyke er enten registrert direkte som nysyke eller, hvis nysykvariabelen er tom, har 30 dager eller mindre mellom diagnosedato og inklusjonsdato. Én pasient kan ha flere oppfølginger.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}smerter.pdf}
\caption{Gjennomsnittlig pasientrapportert smerte ved inklusjon og oppfølging for nysyke pasienter inkludert i \Sexpr{rap_aar-1} og \Sexpr{rap_aar}. Nysyke er enten registrert direkte som nysyke eller, hvis nysykvariabelen er tom, har 30 dager eller mindre mellom diagnosedato og inklusjonsdato. Én pasient kan ha flere oppfølginger.}
\end{figure}
...
Inklusjon2 <- Pasientsvar[, c("PasientGUID", "FormDate",
                              "Prom_Tretthet",
                              "Prom_PasientGlobalSykdomsaktivitet",
                              "Prom_PasientSmerter")] |>
  filter(!is.na(FormDate)) |>
...
prom_oppf_pasientsvar <- Pasientsvar[, c("PasientGUID", "FormDate",
                                         "Prom_Tretthet",
                                         "Prom_PasientGlobalSykdomsaktivitet",
                                         "Prom_PasientSmerter")] |>
  rename(OppfolgingsDato=FormDate,
...
BVAS_debut <- BVAS |>
  ...
  #                      "PasientGlobalSykdomsaktivitet", "Pasientsmerter")],
  #       by = "PasientGUID") |>
  merge(prom_oppf_pasientsvar, by = "PasientGUID") |>
  ...
  merge(Inklusjon2[, c("PasientGUID", "InklusjonDato", "Sykehusnavn", "UnitId",
...
Indikatorer <- Indikatorer |>
  mutate(orgnr = kobl_unitid_shusnavn_norvas$orgnr[match(UnitId, kobl_unitid_shusnavn_norvas$UnitId)],
         context = "caregiver") |>
  select(-UnitId) |>
  dplyr::filter(year >= 2014,
                !is.na(orgnr)) |>
  dplyr::filter(!(ind_id %in% c('norvas_andel_utredet_GCA',
                                'norvas_andel_ulctmr_storkar',
                                'norvas_andel_ctmr_bihuler_anca',
                                'norvas_andel_ct_thorax_anca',
                                'norvas_minimum_2oppf')))
#
# dg_gml <- read.csv2("~/regdata/norvas/beh_kval_2024.csv") |>
#   dplyr::filter(substr(ind_id, 1, 9) == "norvas_dg")

# kobling <- read.csv2("~/regdata/norvas/koblingsfil_2024.csv",
#                      fileEncoding = "Latin1") |>
#   dplyr::filter(UnitId %in% intersect(UnitId, Inklusjon$UnitId))
# dg_ny <- read.csv2(
#   "~/regdata/norvas/DGA_resultat_2025-05-20/DGA_resultat_landet_hf_diag_2025-05-20.csv",
#   fileEncoding = "Latin1") |>
#   dplyr::bind_rows(read.csv2(
#     "~/regdata/norvas/DGA_resultat_2025-05-20/DGA_resultat_hf_2025-05-20.csv",
#     fileEncoding = "Latin1") |> mutate(kode = "alle",
#                                        hf_nr = 0)) |>
#   dplyr::filter(!is.na(hf_nr)) |>
#   dplyr::mutate(
#     var = Begge + Kun_NorVas,
#     diag_gr = case_when(kode %in% c("I776", "M314", "M315_M316") ~ "LVV",
#                         kode %in% c("M301", "M313", "M317") ~ "ANCA",
#                         kode == "alle" ~ "Total")) |>
#   dplyr::summarise(var = sum(var),
#                    denominator = sum(Total),
#                    .by = c(diag_gr, hf_standard)) |>
#   merge(kobling[, c("hf_standard", "UnitId")], by = "hf_standard", all.x = T) |>
#   dplyr::select(-hf_standard) |>
#   dplyr::filter(!is.na(UnitId)) |>
#   # group_by(UnitId) |>
#   # group_modify(~ .x |> janitor::adorn_totals()) |>
#   # dplyr::ungroup() |>
#   dplyr::mutate(
#     ind_id = case_when(diag_gr == "ANCA" ~ "norvas_dg_anca",
#                        diag_gr == "LVV" ~ "norvas_dg_storkar",
#                        diag_gr == "Total" ~ "norvas_dg_alle"),
#     year = 2023,
#     orgnr = kobl_unitid_shusnavn_norvas$orgnr[match(UnitId, kobl_unitid_shusnavn_norvas$UnitId)],
#     context = "caregiver") |>
#   dplyr::select(context, orgnr, year, var, denominator, ind_id)
#
# dg_ny <- dplyr::bind_rows(dg_ny,
#                           dg_ny |> mutate(year = 2024)) |>
...
#                                 'norvas_minimum_2oppf')))
write.csv2(
  Indikatorer |> dplyr::filter(year <= rap_aar),
  paste0("C:/Users/kth200/regdata/norvas/behandlingskvalitet/Indikatorer_", Sys.Date(), ".csv"),
  row.names = F)
...
tid_symptom_diagnose_tabell <- tid_symptom_diagnose_uke |>
  ...
  select(Sykehusnavn, verdi, Diag_gr) |>
  spread(key = Diag_gr, value = verdi)
...
lav_igG <- Labskjema |>
  ...
  rename("Lav IgG" = igG_lav)

print(xtable::xtable(lav_igG, align= c('l','l',rep('r', ncol(lav_igG)-1)), caption=paste0('Antall ANCA-pasienter med IgG $\\leq$ 5 på minst én registrering. N angir antall ANCA-pasienter som har minst en registrering av IgG-verdi. Gjelder t.o.m. ', rap_aar, '.'), digits = 0), include.rownames = F)
...
andel_utfort_igG <- oppfolging_anca |>
  ...
  rename("Antall utført IgG" = ant_igG,
         "Andel utført IgG" = andel_igG)

print(xtable::xtable(
  andel_utfort_igG,
  align= c('l','l',rep('r', ncol(andel_utfort_igG)-1)),
  caption=paste0('Andel oppfølginger av ANCA-pasienter med utført IgG.
                 Gjelder t.o.m. ', rap_aar),
...
inkloppf$Dato[is.na(inkloppf$Dato)] <-
  inkloppf$InklusjonDato[is.na(inkloppf$Dato)]
# Beholder kun anca
inkloppf <- inkloppf |> filter(Diag_gr_nr == 2)

# kobler inn blodprøvesvar og beholder kun registreringer som har registrert igGverdi
inkloppf_lab <- merge(
  inkloppf[, c("PasientGUID", "Sykehusnavn",
               "InklusjonDato", "OppfolgingsDato", "Dato")],
  Labskjema[!is.na(Labskjema$IgGVerdi),
            c("PasientGUID", "Blodprover_Dato", "IgGVerdi")],
  by.x = c('PasientGUID','Dato'),
...
# Sammenfatter per pasient de som på minst en registrering etter
# første rituximabdose har hatt en eller flere registreringer med igG
# mindre enn 5
...
oppsum <- oppsum |>
  ...
  rename("Antall lav IgG" = ant_lav,
         "Andel lav IgG" = andel_lav)

print(xtable::xtable(
  oppsum, align= c('l','l',rep('r', ncol(oppsum)-1)),
  caption=paste0('Andel ANCA-pasienter som har fått minst en rituximabdose
  som på minst en senere oppfølging har registrert IgGVerdi $<$ 5. Kun
  pasienter som har fått rituximab minst en gang og har registrert IgGVerdi
...
print(xtable::xtable(
  oppsum, align= c('l','l',rep('r', ncol(oppsum)-1)),
  caption=paste0('Andel ANCA-pasienter som har fått minst en
  immunoglobindose som på minst en senere oppfølging har registrert
  IgGVerdi $<$ 5. Kun pasienter  som har fått rituximab minst en gang og
  har registrert IgGVerdi minst en gang er inkludert i nevneren. Gjelder
  t.o.m ', rap_aar),
  digits = c(0,0,0,0,1)), include.rownames = F)
...
inkloppf_lavigg <-
  inkloppf_lab[inkloppf_lab$IgGVerdi < 5, ] |>
  merge(
    Alvorlig_infeksjon[, c('PasientGUID',
...
oppsum <- oppsum |>
  ...
  rename("Antall alvorlig infeksjon" = ant_alvorlig,
         "Andel alvorlig infeksjon" = andel_alvorlig)

print(xtable::xtable(
  oppsum, align= c('l','l',rep('r', ncol(oppsum)-1)),
  caption=paste0(
    'Andel ANCA-pasienter med IgGVerdi $<$ 5 som har hatt en eller flere
  alvorlige infeksjoner. Nevneren inkluderer kun pasienter som på samme
  inklusjon/oppfølging der IgGVerdi $<$ 5 også har fylt ut skjema for
  alvorlig infeksjon. Telleren er de som har markert for AntallInfeksjoner
  $>$ 0 på skjema for alvorlig infeksjon. Gjelder t.o.m ', rap_aar),
  digits = c(0,0,0,0,1)), include.rownames = F)
...
<<'Fig. Tid fra inklusjon til oppfølging', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=
tidfrainklusjon<- Inklusjon |>
  filter(Inklusjonsaar <= rap_aar) |>
  dplyr::filter(Diagnose_ny_30 == 1) |>
  select(SkjemaGUID, InklusjonDato, FormStatus) |>
  merge(Oppfolging[, c("HovedskjemaGUID", "OppfolgingsDato",
                       "SkjemaGUID", "UnitId",
                       "FormStatus")],
        by.x = "SkjemaGUID", by.y = "HovedskjemaGUID",
        suffixes = c("", "_oppf")) |>
  mutate(tid_inkl_oppf = difftime(OppfolgingsDato, InklusjonDato,
                                  units = "days") |>
           as.numeric()) |> filter(tid_inkl_oppf >= 0) |>
  mutate(tid_inkl_oppf_kat = cut(tid_inkl_oppf,
                                 breaks = c(0,90,180,270,360, 5000),
                                 include.lowest = TRUE ))

tidfrainklusjon_forste <- tidfrainklusjon |>
  filter(OppfolgingsDato==first(OppfolgingsDato,
                                order_by = OppfolgingsDato),
         .by = SkjemaGUID)

table(tidfrainklusjon$tid_inkl_oppf_kat, useNA = 'ifany')
table(tidfrainklusjon_forste$tid_inkl_oppf_kat, useNA = 'ifany')

aldervar='PatientAge'
valgtVar='tid_inkl_oppf_kat'

outfile <- paste0(figfolder, 'tid_inklu_oppf_alle.', utformat)
norvasFigAndeler(RegData=tidfrainklusjon, valgtVar=valgtVar,
                 datoFra="2000-01-01", datoTil=datoTil,
                 minald=minald, maxald=maxald, erMann=erMann,
                 outfile=outfile, reshID=reshID,
                 enhetsUtvalg=enhetsUtvalg, preprosess=preprosess,
                 valgtShus=valgtShus,hentData=hentData,
                 datovar="InklusjonDato", aldervar=aldervar)

outfile <- paste0(figfolder, 'tid_inklu_oppf_forste.', utformat)
norvasFigAndeler(RegData=tidfrainklusjon_forste, valgtVar=valgtVar,
                 datoFra="2000-01-01", datoTil=datoTil,
                 minald=minald, maxald=maxald, erMann=erMann,
                 outfile=outfile, reshID=reshID,
                 enhetsUtvalg=enhetsUtvalg, preprosess=preprosess,
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}tid_inklu_oppf_alle.pdf}
\caption{Tid fra inklusjon til oppfølging for pasienter inkludert t.o.m. \Sexpr{rap_aar}. Gjelder alle oppfølginger for pasienter som er inkludert nysyke.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}tid_inklu_oppf_forste.pdf}
\caption{Tid fra inklusjon til første oppfølging for pasienter inkludert t.o.m. \Sexpr{rap_aar}. Gjelder  oppfølginger for pasienter som er inkludert nysyke.}
\end{figure}

<<'Indikator 8 og 9: pas med anca med årlig måling av Igg', results='asis', echo=FALSE, warning=F>>=

## inklusjonsskjema for alle anca-pasienter. var = 1 for alle som får
## målt igg ved inklusjon
inkl_anca_igg <- Inklusjon |>
  filter(Diag_gr_nr == 2) |>
  select(PasientGUID, Sykehusnavn, UnitId, InklusjonDato) |>
  merge(Labskjema |> select(PasientGUID, Blodprover_Dato, IgGVerdi),
        by.x = c('PasientGUID','InklusjonDato'),
        by.y = c('PasientGUID', 'Blodprover_Dato'), all.x = TRUE) |>
  mutate(var = as.numeric(!is.na(IgGVerdi)),
         denominator = 1,
         year = lubridate::year(InklusjonDato),
         ind_id = "norvas_aarlig_igg_anca") |>
  select(PasientGUID, UnitId, Sykehusnavn, year, var, denominator, ind_id) |>
  filter(year <= rap_aar)

## Inneholder alle anca-pasienter som har et oppfølgingsskjema et gitt år etter
## inklusjon og før en ev. eksklusjon. var = 1 for alle som får
## målt igg ved oppfølging
oppf_anca_igg <- Oppfolging |>
  filter(Diag_gr_nr == 2) |>
  select(PasientGUID, Sykehusnavn, UnitId, OppfolgingsDato, InklusjonDato) |>
  merge(Oppfolging |> dplyr::select(PasientGUID, EksklusjonsDato) |>
          dplyr::filter(!is.na(EksklusjonsDato)),
        by = "PasientGUID", all.x = TRUE) |>
  filter(lubridate::year(OppfolgingsDato) > lubridate::year(InklusjonDato)) |>
  merge(Labskjema |> select(PasientGUID, Blodprover_Dato, IgGVerdi),
        by.x = c('PasientGUID','OppfolgingsDato'),
        by.y = c('PasientGUID', 'Blodprover_Dato'), all.x = TRUE) |>
  mutate(var = as.numeric(!is.na(IgGVerdi)),
         ...
         Eksklusjonsaar = ifelse(is.na(Eksklusjonsaar), 2050,
                                 Eksklusjonsaar),
         year = lubridate::year(OppfolgingsDato)) |>
  filter(lubridate::year(OppfolgingsDato) < Eksklusjonsaar) |>
  summarise(var = max(var),
            Sykehusnavn = first(Sykehusnavn),
            .by = c(PasientGUID, year, UnitId)) |>
...
aktuelle_anca <- Inklusjon |>
  filter(Diag_gr_nr == 2) |>
  select(UnitId, Sykehusnavn, PasientGUID, Inklusjonsaar, InklusjonDato) |>
  as_tibble()
aux <- aktuelle_anca[1, ]

# For alle pasienter, lag en rad for hvert år pasienten har vært inkludert
# som inneholder UnitId, pasientguid, inklusjonsår og år for potensiell oppfølging
for (id in aktuelle_anca$PasientGUID) {
  for (aar in aktuelle_anca$Inklusjonsaar[
    aktuelle_anca$PasientGUID==id]:rap_aar) {
    aux <- bind_rows(
      aux,
      bind_cols(aktuelle_anca[aktuelle_anca$PasientGUID==id, ],
                Oppfolgingsaar=aar))
  }
}

## Inneholder alle anca-pasienter som er inkludert minst året før og som ennå
## ikke er ekskludert. var = 1 for alle som får målt igg ved oppfølging
oppf_anca_igg_alle <- aux[-1, ] |>
  merge(Oppfolging |> dplyr::select(PasientGUID, EksklusjonsDato) |>
          dplyr::filter(!is.na(EksklusjonsDato)),
        by = "PasientGUID", all.x = TRUE) |>
  dplyr::mutate(Eksklusjonsaar = lubridate::year(EksklusjonsDato),
                Eksklusjonsaar = ifelse(is.na(Eksklusjonsaar), 2050,
                                        Eksklusjonsaar)) |>
  dplyr::filter(Oppfolgingsaar > Inklusjonsaar,
                Oppfolgingsaar < Eksklusjonsaar) |>
  rename(year = Oppfolgingsaar) |>
  merge(oppf_anca_igg |> select(PasientGUID, UnitId, year, var),
        by = c("PasientGUID", "UnitId", "year"),
        all.x = TRUE) |>
...
tab_igg_maalt_anca_inkl <- inkl_anca_igg |>
  filter(year <= rap_aar,
         year > rap_aar-5) |>
  summarise(Antall = sum(var),
            N = n(),
  ...
  mutate(Andel = Antall/N*100,
         Andel = paste0(sprintf("%.1f", Andel), "% (", N, ")"),
         Andel = ifelse(N<5, "N<5", Andel)) |>
  select(Sykehusnavn, year, Andel) |>
  tidyr::pivot_wider(names_from = year, values_from = Andel,
                     names_sort = TRUE) |>
  arrange(Sykehusnavn)


tab_igg_maalt_anca_oppf_v1 <- oppf_anca_igg |>
  filter(year <= rap_aar,
         year > rap_aar-5) |>
  summarise(Antall = sum(var),
            N = n(),
...
tab_igg_maalt_anca_oppf_v2 <- oppf_anca_igg_alle |>
  filter(year <= rap_aar,
         year > rap_aar-5) |>
  summarise(Antall = sum(var),
            N = n(),
...
print(xtable::xtable(
  tab_igg_maalt_anca_inkl, align= c('l','l',rep('r', ncol(tab_igg_maalt_anca_inkl)-1)),
  caption="Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG ved inklusjon."),
  include.rownames = F)

print(xtable::xtable(
  tab_igg_maalt_anca_oppf_v1, align= c('l','l',rep('r', ncol(tab_igg_maalt_anca_oppf_v1)-1)),
  caption="Versjon 1: Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG ved oppfølging. Nevner er alle pasienter som har oppfølging i aktuelle år."),
  include.rownames = F)

print(xtable::xtable(
  tab_igg_maalt_anca_oppf_v2, align= c('l','l',rep('r', ncol(tab_igg_maalt_anca_oppf_v2)-1)),
  caption="Versjon 2: Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG ved oppfølging. Nevner er alle pasienter som skulle hatt oppfølging i aktuelle år."),
  include.rownames = F)
@



<<'Indikator 8 og 9', include=FALSE, echo=FALSE, eval=T, cache=FALSE, warning=F>>=

outfile <- paste0(figfolder, "andel_igg_maalt_inkl.", utformat)
norvasFigIndikatorAar(
  inkl_anca_igg |>
    rename(Aar = year,
           Teller = var),
  tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
             "som får målt serum-IgG ved inklusjon"),
  outfile = outfile)

outfile <- paste0(figfolder, "andel_igg_maalt_oppf_v1.", utformat)
norvasFigIndikatorAar(
  oppf_anca_igg |>
    rename(Aar = year,
           Teller = var),
  tittel = c("Andel pasienter med ANCA-assosiert vaskulitt",
             "som får målt serum-IgG årlig ved oppfølging"),
  outfile = outfile)

outfile <- paste0(figfolder, "andel_igg_maalt_oppf_v2.", utformat)
norvasFigIndikatorAar(
  oppf_anca_igg_alle |>
    rename(Aar = year,
           Teller = var),
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}andel_igg_maalt_inkl.pdf}
\caption{Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG ved inklusjon.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}andel_igg_maalt_oppf_v1.pdf}
\caption{Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG minst årlig ved oppfølging. Blant de som har minst én oppfølging i aktuelle år.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}andel_igg_maalt_oppf_v2.pdf}
\caption{Andel pasienter med ANCA-assosiert vaskulitt som får målt serum-IgG minst årlig ved oppfølging. Blant de som har skulle hatt oppfølging i aktuelle år.}
\end{figure}
...
% # anca_igg_maalt <- dplyr::bind_rows(
% #   inkl_anca_igg,
% #   oppf_anca_igg)
% #
% # tab_igg_maalt_anca <- anca_igg_maalt |>
% #   filter(year <= rap_aar) |>
% #   summarise(Antall = sum(var),
% #             N = n(),
% #             .by = c(Sykehusnavn, year)) |>
