...
Diagnoser_hf_oppf2018 <- table(OppfMedDiagnose[which(OppfMedDiagnose$Oppfolgingsaar==2018), c("Sykehusnavn", "Diag_gr")], useNA = 'ifany') %>% addmargins(2) %>% as.data.frame() %>% spread(key = Sykehusnavn, value = Freq)

# table(Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar < 2018])



teller <- table(OppfMedDiagnose[OppfMedDiagnose$PasientGUID %in% Inklusjon$PasientGUID[Inklusjon$Inklusjonsaar < 2018] & OppfMedDiagnose$Oppfolgingsaar==2018, c("Sykehusnavn", "Diag_gr")])
nevner <- table(Inklusjon[Inklusjon$Inklusjonsaar < 2018, c("Sykehusnavn", "Diag_gr")])
...
bvas_tb <- BVAS_tmp[BVAS_tmp$BVAS_Dato >= '2018-01-01' & BVAS_tmp$BVAS_Dato < '2019-01-01', ] %>% group_by(Sykdomsvurdering, Sykehusnavn) %>% summarise(gjsn.bvas = mean(bvas_samlet),
                                                             N = n())
...
# her kommer alvorlige infeksjoner: Antall for 2018. Samlet og gruppert
