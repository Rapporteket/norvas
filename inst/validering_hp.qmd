...
Det er `{r} dim(Medisin)[1]` registreringer av medisiner fra St. Olavs i Norvas med enten Med_StartDato eller Med_SluttDato i prosjektperioden. Av disse er det en god del duplikater, som definert av at de har samme PasientId, Med_StartDato og Legemiddel.   Det er `{r} dim(Medisin_validering)[1]` registreringer av medisiner i testdatabasen, og også her er det noen tilfeller av samme PasientId, Med_StartDato og Legemiddel. Dette er oppsummert i tabellene under. Det er `{r} sum(Medisin$Med_SluttDato < Medisin$Med_StartDato, na.rm = T)` registreringer med Med_SluttDato før Med_StartDato, i norvas, i testdatabasen er det `{r} sum(Medisin_validering$Med_SluttDato < Medisin_validering$Med_StartDato, na.rm = T)`.
...
knitr::kable(
  oppsum2, 
  caption = "Duplikater av medisiner i testdatabasen", 
  digits = 1,
  align = "l", booktabs = TRUE) |> 
  kableExtra::kable_styling(full_width = FALSE)
...
```
Når man fjerner registreringer med sluttdato før startdato og beholder kun den av de dupliserte registreringene som har flest utfylte felter sitter man igjen med `{r} dim(Medisin)[1]` registreringer i Norvas. I testdatabasen er det `{r} dim(Medisin_validering)[1]` registreringer etter samme filtrering. I Norvas er det variabelen Legemiddel som ser ut til å være utfylt og komplett, mens i testdatabasen registrert direkte i MRS er det LegemiddelType2023 og LegemiddelNr som brukes. 

Det er `{r} sum(Medisin$Legemiddel %in% c("HP ZOLEDRONSYRE 4 MG 100 ML NACL INF", "HP METYLPREDNISOLON INF"))` som er registrert med Legemiddel "HP ZOLEDRONSYRE 4 MG 100 ML NACL INF",  eller "HP METYLPREDNISOLON INF" som ikke finnes i kodebok. Disse fjernes fra den videre sammenstillingen, hvilket betyr at `{r} dim(Medisin_validering)[1] - sum(Medisin$Legemiddel %in% c("HP ZOLEDRONSYRE 4 MG 100 ML NACL INF", "HP METYLPREDNISOLON INF"))` ses på i den videre analysen.
...
Tabellen under viser alle gjenværende registreringer medikamentregistreringer i Norvas og i testdatabasen per pasient og medikament.
...
knitr::kable(
  oppsum_med, 
  caption = "Medikamentregistreringer i Norvas og i testdatabasen", 
  digits = 1,
  align = "l", booktabs = TRUE) |> 
  kableExtra::kable_styling(full_width = FALSE)
