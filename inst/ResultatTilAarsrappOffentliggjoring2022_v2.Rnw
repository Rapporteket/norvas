...
Inklusjon <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_Inklusjonskjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Oppfolging <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_OppfølgingSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Diagnoser <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_DiagnoseSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Medisiner <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_MedisineringSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
BVAS <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_BvasSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
KERR <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_KerrsKriterierSkjema_2023-02-10_1052.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
VDI <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_VdiSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Alvorlig_infeksjon <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_SelvrapportertAlvorligInfeksjonSkjema_2023-02-10_1052.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Utredning <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_Utredning_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
Labskjema <- read.table(
  '~/softlinks/mydata/norvas/DataDump_MRS-PROD_BlodprøvesvarSkjema_2023-02-10_1051.csv',
  header=TRUE, sep=";",
  stringsAsFactors = F, fileEncoding = 'UTF-8-BOM')
...
Inklusjon$Diagnose_ny[Inklusjon$InkludertNyEtablertDiagnose==2] <- 0
Inklusjon$Diagnose_ny[Inklusjon$InkludertNyEtablertDiagnose==1] <- 1
...
grtxt <- registreringer$Sykehusnavn
legendTxt <- norvas::wrap.it(paste0(names(registreringer)[-1], ", N=", colSums(registreringer[, -1])), 34)
norvarStabelGrvar(plotMatrise=plotMatrise,
                  grtxt = grtxt,
                  outfile = outfile,
...
xmax <- min(100, 1.15*max(colSums(plotMatrise)), na.rm=T)
pos <- barplot(plotMatrise, beside = T, horiz = T, col = rev(figinfo$farger[1:dim(plotMatrise)[1]]),
               border=NA, xlab="Andel pasienter", xlim = c(0, xmax))
mtext(at=colMeans(pos)+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
legend('bottomright', legend = c("Kortikosteroider", "DMARD", "Biologiske legemidler"),
       col = figinfo$farger[1:dim(plotMatrise)[1]],
       pch = 15, border = NA, bty='n')
title(main = c('Medikamentbruk ved storkarsvaskulitter'))
if ( outfile != '') {dev.off()}
...
par('fig'=c(vmarg, 1, 0, 1))
xmax <- min(100, 1.15*max(colSums(plotMatrise)), na.rm=T)
pos <- barplot(plotMatrise, beside = F, horiz = T, col = rev(figinfo$farger[1:dim(plotMatrise)[1]]),
               border=NA, xlab="Andel pasienter", xlim = c(0, xmax))
mtext(at=pos+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
legend('bottomright', legend = norvas::wrap.it(paste0(names(ant_pas_pr_med_gr1)[-c(1, dim(ant_pas_pr_med_gr1)[2])], ", N=",
...
plotMatrise <- ant_pas_pr_med_gr2[, rev(c("Kortikosteroider", "DMARD", "Cyclofosfamid", "Rituximab"))]/ant_pas_pr_med_gr2$N*100
...
pos <- barplot(plotMatrise, beside = T, horiz = T, col = rev(figinfo$farger[1:dim(plotMatrise)[1]]),
               border=NA, xlab="Andel pasienter", xlim = c(0, xmax))
mtext(at=colMeans(pos)+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
legend('bottomright', legend = c("Kortikosteroider", "DMARD", "Cyclofosfamid", "Rituximab"),
       col = figinfo$farger[1:dim(plotMatrise)[1]],
       pch = 15, border = NA, bty='n')
title(main = c('Medikamentbruk ved ANCA-assosierte vaskulitter'))
if ( outfile != '') {dev.off()}
...
xmax <- min(100, 1.15*max(plotvektor, na.rm=T))
pos <- barplot(plotvektor[rkflg], horiz = T, col = figinfo$farger[1],
               border=NA, xlab="Andel pasienter (%)", xlim = c(0, xmax))
...
pos <- barplot(plotMatrise, beside = F, horiz = T, col = rev(figinfo$farger[1:dim(plotMatrise)[1]]),
               border=NA, xlab="Andel pasienter", xlim = c(0, xmax))
mtext(at=pos+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
legend('bottomright', legend = norvas::wrap.it(paste0(names(ant_pas_pr_med_gr2)[-c(1, dim(ant_pas_pr_med_gr2)[2])], ", N=",
...
ant_pas_pr_med <- ant_pas_pr_med[order(ant_pas_pr_med$andel_foer_rapaar, decreasing = F, na.last = F), ]
...
vmarg <- min(1,max(0, strwidth(grtxt, units='figure', cex=cexgr)*0.75))
par('fig'=c(vmarg, 1, 0, 1))
xmax <- min(100, 1.15*max(plotMatrise), na.rm=T)
pos <- barplot(plotMatrise, beside = T, horiz = T, col = figinfo$farger[c(1,3)],
               border=NA, xlab="Andel pasienter (%)", xlim = c(0, xmax))
mtext(at=colMeans(pos)+0.00, text=grtxt, side=2, las=1, cex=cexgr, adj=1, line=0.25)
...
ant_pas_pr_med_foer_rapaar <- Medisiner[which(Medisiner$Med_StartDato < datoFra), ] %>%
  group_by(Medikamentgruppe) %>% summarise(Antall = length(unique(PasientGUID))) %>% ungroup()
# N_foer_rapaar <- length(unique(Medisiner[Medisiner$Med_StartDato < '_rapaar-01-01', "PasientGUID"]))
indMedisinert_foer_rapaar <- which(Medisiner$Med_StartDato < datoFra)
pid_Medisinert_foer_rapaar <- unique(Medisiner$PasientGUID[indMedisinert_foer_rapaar])
pid_oppfolging_foer_rapaar <- unique(Oppfolging$PasientGUID[Oppfolging$Oppfolgingsaar<rap_aar])
...
ant_pas_pr_med <- ant_pas_pr_med[order(ant_pas_pr_med$andel_foer_rapaar, decreasing = F), ]
...
nysyke <- Inklusjon[which(abs(difftime(Inklusjon$Diagnose_Klinisk_Dato, Inklusjon$InklusjonDato, units = 'days')) <= 30*6), ]
...
## Lag tabell over
# Inklusjon$InkludertNyEtablertDiagnose
# table(Inklusjon[Inklusjon$Inklusjonsaar==2022, c("Diag_gr", "InkludertNyEtablertDiagnose")])
...
aux <- merge(nysyke, Labskjema, by.x = c('SkjemaGUID', "InklusjonDato"),
  ...
  merge(BVAS, by.x = c('SkjemaGUID', "InklusjonDato"), by.y = c('HovedskjemaGUID', "BVAS_Dato"),
        suffixes = c("", "_bvas")) %>%
  filter(Sykdomsvurdering == 1)

aux2 <- aux[which(aux$Diag_gr_nr.x == 2), ]  # Ser kun på ANCA

tmp2 <- aux2 %>% group_by(Sykehusnavn.x, SkjemaGUID) %>%
  summarise(anca = ((1 %in% PR3AncaPositiv) | (0 %in% PR3AncaPositiv) |
                      (1 %in% MPO_AncaPositiv) | (0 %in% MPO_AncaPositiv))) # Ett resultat per sykehus og skjemaguid,
...
figurdata <- Ind1_ANCAvdebut_NorVas[Ind1_ANCAvdebut_NorVas$Aar <=rap_aar, c(1,6,3,5)]
...
tmp <- remisjon %>% group_by(PasientGUID) %>%
  summarise(N = n(),
            tid_diag_bvas = paste(sort(tid_diag_bvas), collapse = ", "),
            InklusjonDato = InklusjonDato[1])
...
<<'BVAS stat, ved inklusjon', results='asis', echo=FALSE, warning=F>>=
BVAS$tid_diag_bvas <- difftime(BVAS$BVAS_Dato, BVAS$Diagnose_Klinisk_Dato, units = 'days')
InklusjonBVAS <- merge(Inklusjon[,c("Diagnose_ny","InklusjonDato", "Inklusjonsaar", "SkjemaGUID")],
                       BVAS, by.x = c("InklusjonDato", "SkjemaGUID"),
...
# %>%
#   group_by(Sykehus) %>%
#   summarise(Antall_remisjon = sum(remisjon),
#             N=n()) %>%
#   janitor::adorn_totals() %>%
#   mutate(Andel_remisjon = Antall_remisjon/N*100)


# Oppfolging %>% group_by(PasientGUID) %>%
#   summarise()

# aux2 <- aux %>%
#   dplyr::filter(InklusjonDato != BVAS_Dato) %>%
#   dplyr::filter(BVAS_Dato > Diagnose_Klinisk_Dato) %>%
#
#   aux[aux$InklusjonDato != aux$BVAS_Dato, ]

# InklusjonVDI <- merge(Inklusjon, VDI, by.x = c("InklusjonDato", "SkjemaGUID"),
#                        by.y = c("VDI_Dato", "HovedskjemaGUID"))

# table(InklusjonBVAS$SykdomsvurderingLabel)
# Inklusjon$SkjemaGUID
# BVAS$BVAS_Dato
@
...
print(xtable::xtable(andel_remisjon_6mnd, align= c('l','l','r','r'), digits=c(0,0,1,0), caption=paste0('Andel i remisjon innen 6 måneder fra diagnosedato ut fra Kerrs for storkarsvaskulitter t.o.m. ', rap_aar, '. Gjelder nysyke som definert ved at KerrsKriterier\\_Dato ved debut og inklusjonsdato er innenfor plussminus 30 dager.')), include.rownames = F)
...
aux <- merge(nysyke, Utredning, by = 'SkjemaGUID', by.y = 'HovedskjemaGUID', all.x = T)
aux <- aux[aux$Navn == "Kjempecellearteritt", ]
...
print(xtable::xtable(andel_utfort_igG, align= c('l','l',rep('r', ncol(andel_utfort_igG)-1)),
                     caption=paste0('Andel oppfølginger med utført igG. Gjelder t.o.m. ', rap_aar),
...
# anca_igg_ritux <- merge(inkloppf_lab, med_ritux_min, by = "PasientGUID", all.x = T)
# anca_igg_ritux <- anca_igg_ritux[!is.na(anca_igg_ritux$med_dato), ]
...
# med_ritux_nested <- med_ritux[, c("PasientGUID", "Med_StartDato", "Med_SluttDato")] %>% group_by(PasientGUID) %>% nest()
# tmp2 <- med_ritux[, c("PasientGUID", "Med_StartDato", "Med_SluttDato")] %>% nest_by(PasientGUID)


# tmp <- merge(Alvorlig_infeksjon, Oppfolging,
#              by.x = c('PasientGUID','SelvrapportertAlvorligInfeksjon_Registrert_Dato'),
#                          by.y = c('PasientGUID', 'OppfolgingsDato'))
# tmp2 <- merge(Alvorlig_infeksjon, Inklusjon,
#              by.x = c('PasientGUID','SelvrapportertAlvorligInfeksjon_Registrert_Dato'),
#                          by.y = c('PasientGUID', 'InklusjonDato'))
#
# kobletdata <- Inklusjon %>%
#   dplyr::filter(Diag_gr_nr == 2) %>%
#   merge()


# Inklusjon_anca <- Inklusjon[which(Inklusjon$Diag_gr_nr == 2 &
#                                       Inklusjon$Inklusjonsaar <= rap_aar), ]
# Inklusjon_anca <- merge(Inklusjon_anca, Labskjema[, c("PasientGUID", "Blodprover_Dato", "IgGVerdi")],
#                          by.x = c('PasientGUID','InklusjonDato'),
#                          by.y = c('PasientGUID', 'Blodprover_Dato'), all.x = T)
#
# andel_utfort_igG <- Inklusjon_anca %>%
#   group_by(Sykehusnavn) %>%
#   summarise(ant_igG = sum(!is.na(IgGVerdi)),
#             N = n(),
#             andel_igG = ant_igG/N*100)
