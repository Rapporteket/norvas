...
# aux2 <- aux[which(abs(difftime(aux$Diagnose_Klinisk_Dato.x, aux$Blodprover_Dato, units = 'days')) <= 30*6), ] # Avgrenser til tilfeller med
...
tull <- aux2 %>% group_by(Sykehusnavn.x) %>%
  summarise(ant_anca = sum(as.numeric(((1 %in% PR3AncaPositiv) | (0 %in% PR3AncaPositiv) |
                      (1 %in% MPO_AncaPositiv) | (0 %in% MPO_AncaPositiv)))),
            N = n())
