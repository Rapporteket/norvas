...
#' @export
#'
norvasFigIndikator <- function(indikatordata, tittel='', terskel=5, minstekrav = NA, maal = NA, skriftStr=1.3, pktStr=1.4,
                             ...
                             lavDG=NA, width=800, height=700, inkl_konf=F, maalretn='hoy')
{
  ...
  Tabell <- indikatordata %>% dplyr::group_by(Sykehusnavn, Aar) %>%
    dplyr::summarise(Antall = sum(Teller),
                     N = dplyr::n(),
                     Andel = Antall/N*100) %>%
    dplyr::mutate(Sykehusnavn = as.character(Sykehusnavn))
